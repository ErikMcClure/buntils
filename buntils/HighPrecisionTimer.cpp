...
HighPrecisionTimer::HighPrecisionTimer() : _time(0), _nsTime(0) { ResetDelta(); }
...
double HighPrecisionTimer::Update()
{
  ...
  _queryTime(&newTime);
  if(newTime < _curTime)
    newTime = _curTime; // Do not allow time to run backwards
#ifdef BUN_PLATFORM_WIN32
  _delta = double((newTime - _curTime) * 1000) /
           (double)hpt_freq; // We multiply by 1000 BEFORE dividing into a double to maintain precision (since its unlikely
                             // the difference between newtime and oldtime is going to be bigger then 9223372036854775
...
{ // The multicore timing glitch that used to happen with QPC calls no longer affects modern windows. See:
