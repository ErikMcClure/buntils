...
//{
//  char ibuf[MAXLINELENGTH]; //we actually ignore max here because we know it will be this - we accept it so this is
//  compatible with the other call fgets(ibuf,--max,file); //we subtract one from max so we have room for the null
//  terminator in the unicode string max=UTF8Decode2BytesUnicode(ibuf,buf); buf[max]=0;
//}
...
char bun_InitINI(INIParser* init, FILE* stream)
{
  if(!init)
    return 0;
  init->file     = stream;
  init->curvalue = (char*)malloc(sizeof(char)); // If this isn't sizeof(char), when the next line gets executed, the system
                                                // thinks its 2 bytes long for wchar_t and overwrites memory
  if(!init->curvalue)
    return 0;
  *init->curvalue = 0;
...
char _nextkeychar(const char** output, const char* line)
{
  for(; *line; ++line)
  {
    ...
    {
    case ';': return -1;
    case '=': *output = line; return 0;
    case '[': *output = line; return 1;
    }
  }
  return -1;
}
...
char* _validatesection(char* start)
{
  ...
  {
    if(*start == ';')
      return 0; // a comment started before we found the end
    if(*start == ']')
      break; // found it, break to end of function
    if(valid == 0 && *start >= VALIDASCII)
      valid = 1;
  }
...
char _validatekey(const char* start, const char* end)
{
  for(; start < end; ++start)
    if(*start >= VALIDASCII)
      return 1; // 32 is space and everything below is just crap.
  return 0;
}

// Finds the end of a value line (before a comment starts) but doesn't care about whether the value has valid characters or
// not.
...
char _snextkeychar(const char** output, const char* start, const char* end)
{
  for(; start < end; ++start)
  {
    ...
    {
    case ';': return -1;
    case '=': *output = start; return 0;
    case '[': *output = start; return 1;
    }
  }
  return -1;
}
...
INICHUNK bun_FindINISection(const void* data, size_t length, const char* section, size_t instance)
{
  ...
  while(cur < end)
  {
    line = strchr(cur, '\n');
    if(!line)
      line = end; // if that returned nothing we're on the last line
    if(_snextkeychar(&tc, cur, line) == 1)
    {
      ...
      {
        retval.start = tc; // this is safe because we already verified that there is at least one valid character in this
                           // string. Ok, it still isn't safe, but it won't explode.
  ...
  while(cur < end)
  {
    ...
    if(_snextkeychar(&tc, cur, line) == 0)
    {
      if(_validatekey(cur, tc) != 0 && comparevalues(cur, tc, key) == 0 && (++curinstance) == instance)
      {
        ...
        retval.start = _trimlstr(cur); // this is safe because we already verified that there is at least one valid
                                       // character in this string. Ok, it still isn't safe, but it won't explode.
        retval.end = sec;
        break;
      }
    }
