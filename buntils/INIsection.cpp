...
LocklessBlockPolicy<INIsection::_NODE> INIsection::_alloc;

INIsection::INIsection(const INIsection& copy) :
  _name(copy._name), _index(copy._index), _parent(copy._parent), _root(0), _last(0)
{
  _copy(copy);
}
INIsection::INIsection(INIsection&& mov) :
...
INIsection::INIsection(const char* name, INIstorage* parent, size_t index) :
  _name(name), _index(index), _parent(parent), _root(0), _last(0)
{}
INIsection::~INIsection() { _destroy(); }
...
{ // We put this down here because the compiler never actually inlines it, so there's no point in going through hoops to
  // keep it inline.
