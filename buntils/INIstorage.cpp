...
template<typename T> const T* ltrimstr(const T* str)
{
...
template<typename T> const T* rtrimstr(const T* end, const T* begin)
{
...
INIstorage::INIstorage(const INIstorage& copy) :
  _path(copy._path),
  _filename(copy._filename),
  _ini(!copy._ini ? 0 : new Str(*copy._ini)),
...
INIstorage::INIstorage(INIstorage&& mov) :
  _path(std::move(mov._path)),
  _filename(std::move(mov._filename)),
  _ini(mov._ini),
  _logger(mov._logger),
...
INIstorage::INIstorage(FILE* file, std::ostream* logger) : _ini(0), _logger(logger), _root(0), _last(0) { _loadINI(file); }
...
INIsection::_NODE* INIstorage::GetEntryNode(const char* section, const char* key, size_t keyinstance,
                                            size_t secinstance) const
{
  INIsection* s = GetSection(section, secinstance);
...
INIsection* INIstorage::_addSection(const char* name)
{
  ...
  else
  {
    ...
    new(&p->val)
      INIsection(name, this,
                 r->instances.Capacity()); // done down here because the index is actually where it is in the array + 1.
  }
...
bool INIstorage::RemoveSection(const char* name, size_t instance)
{
  if(!_ini)
    _openINI();
  INICHUNK chunk = bun_FindINISection(*_ini, _ini->size(), name, instance);
  khiter_t iter  = _sections.Iterator(name);
  ...
  if(iter != _sections.Back() && chunk.start != 0)
  {
    ...
    if(instance > secnode->instances.Capacity())
      return false; // if keyinstance is not valid, fail
    if(instance != 0)
      secnode = secnode->instances[instance - 1];

    // If you are deleting a root node that has danglers, we just replace the root with one of the danglers and transform it
    // into a dangler case.
    ...
    if(!instance) // If this is true you are a root, but you don't have any danglers (see above check), so we just remove
                  // you from the hash.
    ...
    { // Otherwise you are a dangler, so all we have to do is remove you from the instances array
      ...
      for(; instance < secroot->instances.Capacity(); ++instance)
        --secroot->instances[instance]->val._index; // moves all the indices down
    }
...
char INIstorage::EditEntry(const char* section, const char* key, const char* nvalue, size_t keyinstance, size_t secinstance)
{
  ...
  if(iter == _sections.Back())
    return -1; // if it doesn't exist at this point, fail
  _NODE* secnode = _sections.Value(iter);
  // if(secinstance==(size_t)~0) secinstance=secnode->instances.Capacity()-1; //This is now done at the start of the function
  if(secinstance > secnode->instances.Capacity())
    return -2; // if secinstance is not valid, fail
  if(secinstance > 0)
    secnode = secnode->instances[secinstance - 1];

  INIsection* psec = &secnode->val;
  INICHUNK chunk   = bun_FindINISection(_ini->c_str(), _ini->size(), section, secinstance);
  if(!chunk.start)
    return -3; // if we can't find it in the INI, fail

  if(keyinstance == (size_t)~0) // insertion
  {
    psec->_addEntry(key, nvalue);
    //_ini->reserve(_ini->size()+strlen(key)+strlen(nvalue)+2); // This is incredibly stupid. It invalidates all our
    //pointers causing strange horrifying bugs.
  ...
  if(iter == psec->_entries.Back())
    return -4; // if key doesn't exist at this point, fail
  _SNODE* entnode = psec->_entries.Value(iter);
  _SNODE* entroot = entnode;
  if(keyinstance > entnode->instances.Capacity())
    return -5; // if keyinstance is not valid, fail
  if(keyinstance != 0)
    entnode = entnode->instances[keyinstance - 1];
  chunk = bun_FindINIEntry(chunk, key, keyinstance);
  if(!chunk.start)
    return -7; // if we can't find it
  ...
  if(!nvalue) // deletion
  {
    ...
    if(!keyinstance) // If this is true you are a root, but you don't have any danglers (see above check), so we just remove
                     // you from the hash.
  ...
  else // edit
  {
    ...
    if(!start)
      return -6; // if this happens something is borked
...
void INIstorage::EndINIEdit(const char* overridepath)
{
  if(!_ini)
    return;
  if(overridepath != 0)
    _setFilePath(overridepath);
  Str targetpath = _path;
  targetpath += _filename;
  FILE* f;
  FOPEN(f, targetpath, ("wb"));
  if(!f)
    return; // IF the file fails, bail out and do not discard the edit.
...
void INIstorage::_copy(const INIstorage& copy)
{
  ...
  while(t)
  {
    ...
    {
      assert(last != 0);
      last->instances[last->instances.Capacity() - (c--) - 1] =
        p; // This never goes negative because c>0 and is therefore at least 1
    }
    else
      _sections.Insert(p->val.GetName(), p);
