...
namespace bun {
  namespace internal {
    struct PROF_HEATNODE
    {
      ...
      std::partial_ordering operator<=>(const PROF_HEATNODE& r) const { return r.avg <=> avg; }
      bool operator==(const PROF_HEATNODE& r) const { return r.avg == avg; }
      ...
      PROF_HEATNODE& operator=(PROF_HEATNODE&& mov)
      {
      ...
      PROF_HEATNODE& operator=(const PROF_HEATNODE& copy)
      {
...
Profiler::Profiler() : _alloc(32), _data(1), _totalnodes(0)
{
  _trie = _cur = _allocNode();
  _data[0]     = 0;
}
...
void Profiler::WriteToStream(std::ostream& stream, uint8_t output)
{
  ...
  if(output & OUTPUT_FLAT)
  {
    ...
    std::sort(avg + 1, avg + _data.Capacity(),
              [](const PROF_FLATOUT& l, const PROF_FLATOUT& r) -> bool { return l.avg > r.avg; });
...
void Profiler::_heatOut(PROF_HEATNODE& heat, PROF_TRIENODE* node, ProfilerInt id, ProfilerInt idlevel)
{
  ...
  if(node->total != (uint64_t)-1)
  {
    ...
    for(auto& child : nheat._children)
      t += child.avg;

    if(nheat._children.size() > 0)
...
double Profiler::_heatFindMax(const PROF_HEATNODE& heat)
{
  double max = heat.avg;
  for(auto& child : heat._children)
    max = std::max(_heatFindMax(child), max);
  return max;
}
void Profiler::_heatWrite(std::ostream& stream, const PROF_HEATNODE& node, size_t level, double max)
{
  ...
  if(node._children.size() == 1 && !node._children[0].id)
    return;
  for(auto& child : node._children)
    _heatWrite(stream, child, level + 1, max);
}
void Profiler::_flatOut(PROF_FLATOUT* avg, PROF_TRIENODE* node, ProfilerInt id, ProfilerInt idlevel)
{
  ...
  if(node->total != (uint64_t)-1)
  {
    ...
    {     // Equation: new_avg = a1 * (N1/(N1+N2)) + a2 * (N2/(N1+N2))
      double navg = avg[id].avg * (avg[id].total / ((double)(avg[id].total + node->total))) +
                    node->avg * (node->total / ((double)(avg[id].total + node->total)));
      // var[id] = avg[id].var + node->variance + avg[id].total*(avg[id].avg - navg)*(avg[id].avg - navg) +
      // node->total*(node->avg - navg)*(node->avg - navg);
      avg[id].avg = navg;
...
{
  // double sd = bun::FastSqrt(variance/(double)(num-1));
