...
namespace bss {
  ...
  public:
    ...
    inline ~ArraySort() {}
    BSS_FORCEINLINE CT Insert(constref item) { CT loc = _insert(item); _array.Insert(item, loc); return loc; }
    BSS_FORCEINLINE CT Insert(moveref item)
    { 
    ...
    BSS_FORCEINLINE void SetCapacity(CT capacity) { _array.SetCapacity(capacity); }
    BSS_FORCEINLINE CT Length() const { return _array.Length(); }
    BSS_FORCEINLINE CT Capacity() const { return _array.Capacity(); }
    ...
    BSS_FORCEINLINE Slice<T, CT> GetSlice() const noexcept { return _array.GetSlice(); }
    ...
    BSS_FORCEINLINE CT Find(constref item) const
    {
    ...
    BSS_FORCEINLINE constref operator [](CT index) const { return _array[index]; }
    BSS_FORCEINLINE T& operator [](CT index) { return _array[index]; }
