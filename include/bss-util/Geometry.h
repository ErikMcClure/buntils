...
#ifndef __BSS_GEOMETRY_H__
#define __BSS_GEOMETRY_H__
...
namespace bss {
  ...
  struct NSphere<T, 2>
  {
    ...
    inline T Diameter() const { return r * 2; }
    inline T Radius() const { return r; }
    BSS_FORCEINLINE bool ContainsPoint(const Vector<T, 2>& p) const { return RadiusContainsPoint(r, p.x - x, p.y - y); }
    BSS_FORCEINLINE bool ContainsPoint(T X, T Y) const { return RadiusContainsPoint(r, X - x, Y - y); }
    BSS_FORCEINLINE bool ContainsCircle(const NSphere<T, 2>& s) const { return ContainsCircle(s.x, s.y, s.r); }
    BSS_FORCEINLINE bool ContainsCircle(const Vector<T, 2>& C, T R) const { return ContainsCircle(C.x, C.y, R); }
    BSS_FORCEINLINE bool ContainsCircle(T X, T Y, T R) const { if(r <= R) return false; return RadiusContainsPoint(r - R, X - x, Y - y); }
    BSS_FORCEINLINE void NearestPoint(const Vector<T, 2>& p, T& outX, T& outY) const { RadiusNearestPoint(r, p.x - x, p.y - y, outX, outY); }
    BSS_FORCEINLINE void NearestPoint(T X, T Y, T& outX, T& outY) const { RadiusNearestPoint(r, X - x, Y - y, outX, outY); }
    BSS_FORCEINLINE int CircleIntersect(T X, T Y, T R, Vector<T, 2>(&out)[2]) const { return CircleRadiusIntersect<T>(r, X - x, Y - y, R, out); }
    BSS_FORCEINLINE bool CircleCollide(T X, T Y, T R) const { return RadiusContainsPoint(r + R, X - x, Y - y); }
    BSS_FORCEINLINE bool CircleCollide(const Vector<T, 2>& C, T R) const { return RadiusContainsPoint(r + R, C.x - x, C.x - y); }
    BSS_FORCEINLINE bool LineCollide(T X1, T Y1, T X2, T Y2) const { return LineRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r); }
    BSS_FORCEINLINE bool RayCollide(T X1, T Y1, T X2, T Y2) const { return RayRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r); }
    BSS_FORCEINLINE bool LineSegmentCollide(T X1, T Y1, T X2, T Y2) const { return LineSegmentRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r); }
    BSS_FORCEINLINE bool LineIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2>(&out)[2]) const { return LineRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out); }
    BSS_FORCEINLINE bool RayIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2>(&out)[2]) const { return RayRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out); }
    BSS_FORCEINLINE bool LineSegmentIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2>(&out)[2]) const { return LineSegmentRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out); }
    BSS_FORCEINLINE bool RectCollide(T left, T top, T right, T bottom) const { return RectCircleCollide<T>(left, top, right, bottom, x, y, r); }
    BSS_FORCEINLINE bool EllipseCollide(T X, T Y, T A, T B) const { return EllipseCircleCollide(X, Y, A, B, x, y, r); }
  ...
  struct LineN<T, 2>
  {
    ...
    BSS_FORCEINLINE LineN Rotate(T r, T x, T y) const { return LineN(p1.Rotate(r, x, y), p2.Rotate(r, x, y)); }
  ...
  struct Rect
  {
    ...
    BSS_FORCEINLINE V Dim() const { return V(right - left, bottom - top); }
    BSS_FORCEINLINE Rect Inflate(T amount) const { return Rect(left - amount, top - amount, right + amount, bottom + amount); }
    BSS_FORCEINLINE Rect Project(T Z) const { return Rect(*this) / (Z + 1); }
    BSS_FORCEINLINE sseVecT<T> ToSSE() const { return sseVecT<T>(ltrb); }
    // Returns true if left <= right and top <= bottom
    BSS_FORCEINLINE bool IsCanonical() const { return left <= right && top <= bottom; }
    // Returns a rectangle with a proper LTRB description, such that left < right and top < bottom.
    BSS_FORCEINLINE Rect Canonical() const { return Rect(bssmin(left, right), bssmin(top, bottom), bssmax(left, right), bssmax(top, bottom)); }
    // Returns a new rectangle that includes the given point.
    BSS_FORCEINLINE Rect ExpandTo(const V& point) const { return Rect(bssmin(left, point.x), bssmin(top, point.y), bssmax(right, point.x), bssmax(bottom, point.y)); }
    // Returns a rectangle that contains both this one and rect
    BSS_FORCEINLINE Rect Union(const Rect& rect) const { return Rect(bssmin(left, rect.left), bssmin(top, rect.top), bssmax(right, rect.right), bssmax(bottom, rect.bottom)); }
    // Returns a rectangle that contains only the overlapping parts (will be degenerate if there is no overlap)
    BSS_FORCEINLINE Rect Intersection(const Rect& rect) const { if(!RectCollide(rect)) return Rect(left, top, left, top); return Rect(bssmax(left, rect.left), bssmax(top, rect.top), bssmin(right, rect.right), bssmin(bottom, rect.bottom)); }

    BSS_FORCEINLINE bool ContainsPoint(T X, T Y) const { return RectContainsPoint(left, top, right, bottom, X, Y); }
    BSS_FORCEINLINE bool ContainsRect(T Left, T Top, T Right, T Bottom) const { return RectContainsRect(left, top, right, bottom, Left, Top, Right, Bottom); }
    BSS_FORCEINLINE bool ContainsRect(const Rect& r) const { return RectContainsRect(left, top, right, bottom, r.left, r.top, r.right, r.bottom); }
    BSS_FORCEINLINE bool CircleCollide(T X, T Y, T R) const { return RectCircleCollide<T>(left, top, right, bottom, X, Y, R); }
    BSS_FORCEINLINE bool CircleCollide(const V& c, T R) const { return RectCircleCollide<T>(left, top, right, bottom, c.x, c.y, R); }
  ...
  struct Ellipse
  {
    ...
    BSS_FORCEINLINE bool RectCollide(T left, T top, T right, T bottom) const { return EllipseRectCollide(x, y, a, b, left, top, right, bottom); }
  ...
  struct CircleSector
  {
    ...
    static BSS_FORCEINLINE bool _checkBothAngles(T X, T Y, int n, const Vector<T, 2>(&in)[2], T min, T range, T Min, T Range)
    {
      for(int i = 0; i < n; ++i)
    ...
    static BSS_FORCEINLINE bool _checkAngles(int n, const Vector<T, 2>(&in)[2], T min, T range)
    {
      for(int i = 0; i < n; ++i)
