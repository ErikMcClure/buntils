...
namespace bss {
  ...
  template<class T>
  struct SerializerSwapper
  {
    explicit SerializerSwapper(T v) : value(v) {}
  ...
  public:
    ...
    template<class T> using ActionBind = Action<T, std::is_arithmetic<T>::value>;

    template<typename... Args>
    struct KeyValueFind
    {
      KeyValueFind(const Trie<uint16_t>& t, const std::tuple<Args...>& args) : _t(t), _args(args) {}
      BSS_FORCEINLINE void operator()(Serializer<Engine>& e, const char* id) { Serializer<Engine>::FindParse<Args...>(e, id, _t, _args); }
    ...
    template<class Key, class Data, khint_t(*__hash_func)(const Key&), bool(*__hash_equal)(const Key&, const Key&), ARRAY_TYPE ArrayType, typename Alloc>
    struct KeyValueInsert
    {
      ...
      BSS_FORCEINLINE void operator()(Serializer<Engine>& e, const char* id)
      {
        Data* v = _obj[id];
        if(!v)
          v = _obj.GetValue(_obj.Insert(id, Data()));
        if(v)
          ActionBind<Data>::template Parse(e, *v, id);
      }
    ...
    void Serialize(const T& obj, std::ostream& s, const char* name = 0)
    {
      out = &s;
      in = 0;
      ActionBind<T>::template Serialize(*this, obj, name);
    }
    ...
    void Parse(T& obj, std::istream& s, const char* name = 0)
    {
      out = 0;
      in = &s;
      ActionBind<T>::template Parse(*this, obj, name);
    }
    ...
    inline void EvaluateType(std::pair<const char*, Args&>... args)
    {
      ...
      if(out) // Serializing
        int X[] = { (ActionBind<Args>::template Serialize(*this, args.second, args.first), 0)... };

      if(in) // Parsing
      {
        if(Engine::Ordered())
          int X[] = { (ActionBind<Args>::template Parse(*this, args.second, args.first), 0)... };
        else
        {
          auto tmp = std::make_tuple<std::pair<const char*, Args&>...>(std::move(args)...);
          Engine::template ParseMany<KeyValueFind<std::pair<const char*, Args&>...>>(*this, KeyValueFind<std::pair<const char*, Args&>...>(t, tmp));
        }
      }
    }
    ...
    struct r_findparse {
      inline static void f(Serializer<Engine>& e, uint16_t index, const std::tuple<Args...>& args)
      {
        ...
        ActionBind<typename std::remove_reference<typename std::tuple_element<I, std::tuple<Args...>>::type::second_type>::type>::template Parse(e, std::get<I>(args).second, std::get<I>(args).first);
      }
    };
    template<typename... Args>
    ...
    template<class T>
    struct Action<T, true> // Integer or float value (seperated to simplify template overloads)
    {
    ...
    template<>
    struct Action<bool, true> // Boolean value (is classified as an integer and must be captured here)
    {
      static inline void Parse(Serializer<Engine>& e, bool& t, const char* id) { Engine::template ParseBool(e, t, id); }
      static inline void Serialize(Serializer<Engine>& e, bool t, const char* id) { Engine::template SerializeBool(e, t, id); }
    };

    template<class STORE>
    struct Action<internal::_BIT_REF<STORE>, false> // Bit reference used in some classes
    {
      static inline void Parse(Serializer<Engine>& e, internal::_BIT_REF<STORE>& t, const char* id) { bool b = t; Engine::template ParseBool(e, b, id); t = b; }
      static inline void Serialize(Serializer<Engine>& e, internal::_BIT_REF<STORE> t, const char* id) { Engine::template SerializeBool(e, t, id); }
    };
    ...
    struct Action<T[I], B>
    {
      ...
      static inline void Add(Serializer<Engine>& e, T(&obj)[I], int& n) { if(n < I) ActionBind<T>::template Parse(e, obj[n++], 0); }
      static inline void Parse(Serializer<Engine>& e, T(&obj)[I], const char* id) { Engine::template ParseArray<T[I], T, &Last, &Add>(e, obj, id); }
    ...
    template<class T, size_t I, bool B> // For fixed-length arrays
    struct Action<std::array<T, I>, B>
    {
      static inline T& Last(std::array<T, I>& obj) { return obj[0]; }
      static inline void Add(Serializer<Engine>& e, std::array<T, I>& obj, int& n) { if(n < I) ActionBind<T>::template Parse(e, obj[n++], 0); }
      static inline void Parse(Serializer<Engine>& e, std::array<T, I>& obj, const char* id) { Engine::template ParseArray<std::array<T, I>, T, &Last, &Add>(e, obj, id); }
    ...
    struct Action<std::vector<T, Alloc>, false>
    {
      ...
      {
        obj.push_back(T());
        ActionBind<T>::template Parse(e, obj.back(), 0);
      }
      static inline void Parse(Serializer<Engine>& e, std::vector<T, Alloc>& obj, const char* id)
      {
        Engine::template ParseArray<std::vector<T, Alloc>, T, &Last, &Add>(e, obj, id);
      }
    ...
    template<class E, typename CType, ARRAY_TYPE ArrayType, typename Alloc>
    struct Action<DynArray<E, CType, ArrayType, Alloc>, false>
    {
      ...
      {
        obj.AddConstruct();
        ActionBind<E>::template Parse(e, obj.Back(), 0);
      }
      static inline void Parse(Serializer<Engine>& e, DynArray<E, CType, ArrayType, Alloc>& obj, const char* id)
      {
        Engine::template ParseArray<DynArray<E, CType, ArrayType, Alloc>, E, &Last, &Add>(e, obj, id);
      }
    ...
    struct Action<DynArray<bool, CType, ArrayType, Alloc>, false>
    {
      ...
      {
        bool b;
        ActionBind<bool>::template Parse(e, b, 0);
        obj.Add(b);
      }
      static inline void Parse(Serializer<Engine>& e, DynArray<bool, CType, ArrayType, Alloc>& obj, const char* id)
      {
        Engine::template ParseArray<DynArray<bool, CType, ArrayType, Alloc>, bool, &Last, &Add>(e, obj, id);
      }
    ...
    struct Action<Array<T, CType, ArrayType, Alloc>, false>
    {
      ...
      {
        Engine::template ParseArray<Array<T, CType, ArrayType, Alloc>, T, &Last, &Add>(e, obj, id);
      }
    ...
    template<class Key, class Data, khint_t(*__hash_func)(const Key&), bool(*__hash_equal)(const Key&, const Key&), ARRAY_TYPE ArrayType, typename Alloc>
    struct Action<HashBase<Key, Data, false, __hash_func, __hash_equal, ArrayType, Alloc>, false> // Hashes that are just sets can be treated as arrays.
    {
      ...
      {
        Key key;
        ActionBind<Key>::template Parse(e, key, 0);
        obj.Insert(std::move(key));
      }
      static inline void Parse(Serializer<Engine>& e, E& obj, const char* id)
      {
        Engine::template ParseArray<E, Key, &Last, &Add>(e, obj, id);
      }
    ...
    template<class Key, class Data, khint_t(*__hash_func)(const Key&), bool(*__hash_equal)(const Key&, const Key&), ARRAY_TYPE ArrayType, typename Alloc>
    struct Action<HashBase<Key, Data, true, __hash_func, __hash_equal, ArrayType, Alloc>, false>
    {
      typedef HashBase<Key, Data, true, __hash_func, __hash_equal, ArrayType, Alloc> E;
      typedef KeyValueInsert<Key, Data, __hash_func, __hash_equal, ArrayType, Alloc> F;
    ...
    template<typename K, typename T, bool ins, ARRAY_TYPE ArrayType, typename Alloc>
    struct Action<Hash<K, T, ins, ArrayType, Alloc>, false> : Action<typename Hash<K, T, ins, ArrayType, Alloc>::BASE, false> {};
    ...
    struct __ActionVariant
    {
      static inline void Parse(Serializer<Engine>& e, T& obj, const char* id)
      {
        if(obj.is<Arg>())
        {
          ...
          obj.typeset<Arg>(); // Properly construct type
          ActionBind<Arg>::template Parse(e, obj.get<Arg>(), id);
        }
        else
          __ActionVariant<T, Args...>::Parse(e, obj, id);
      }
      static inline void Serialize(Serializer<Engine>& e, const T& obj, const char* id)
      {
        if(obj.is<Arg>())
          ActionBind<Arg>::template Serialize(e, obj.get<Arg>(), id);
        else
          __ActionVariant<T, Args...>::Serialize(e, obj, id);
      }
    };
    template<class T, typename Arg>
    struct __ActionVariant<T, Arg>
    {
      static inline void Parse(Serializer<Engine>& e, T& obj, const char* id)
      {
        if(obj.is<Arg>())
