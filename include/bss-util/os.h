...
namespace bss {
  ...
  BSS_COMPILER_DLLEXPORT extern int _listdir(const wchar_t* cdir, void(*fn)(const wchar_t*, std::vector<Str>*), std::vector<Str>* files, char flags);
  static void _listdir_r(const wchar_t* f, std::vector<Str>* files) { files->push_back(f); } // a stupidly roundabout way of sidestepping the DLL boundary problem
  BSS_FORCEINLINE int ListDirW(const wchar_t* dir, std::vector<Str>& files, char flags) { return _listdir(dir, &_listdir_r, &files, flags); } // Setting flags to 1 will do a recursive search. Setting flags to 2 will return directory+file names.
  BSS_FORCEINLINE int ListDir(const char* dir, std::vector<Str>& files, char flags) { return ListDirW(BSSPOSIX_WCHAR(dir), files, flags); }   // Setting flags to 3 will both be recursive and return directory names.
