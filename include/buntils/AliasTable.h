...
namespace bun {
  // Implementation of the Alias method, based off Keith Schwarz's code, found here:
  // http://www.keithschwarz.com/darts-dice-coins/
  template<typename UINT = size_t, typename F = double> class AliasTable
  {
  public:
    AliasTable(const AliasTable& copy) :
      _alias(new UINT[copy._count]), _prob(new F[copy._count]), _count(copy._count), _dist(copy._dist), _fdist(copy._fdist)
    {
      memcpy(_alias, copy._alias, sizeof(UINT) * copy._count);
      memcpy(_prob, copy._prob, sizeof(UINT) * copy._count);
    }
    AliasTable(AliasTable&& mov) :
    ...
    AliasTable(const F* problist, UINT count) : _alias(0), _prob(0), _count(0), _fdist(0, (F)1)
    {
      ...
      UINT g;
      while(n_small != 0 &&
            n_large != 0) // In a perfect world, small always empties before large, but our world isn't, so we check both
      {
        l = small[--n_small];
        g = large[--n_large];
    ...
    template<UINT I> explicit AliasTable(const F (&problist)[I]) : AliasTable(problist, I) {}
    template<UINT I> explicit AliasTable(const std::array<F, I>& problist) : AliasTable(problist.data(), I) {}
    ~AliasTable() {}
    // Gets a new random integer using ENGINE (defaults to xorshift) over a uniform integer distribution
    template<typename ENGINE> BUN_FORCEINLINE UINT Get(ENGINE& e) const
    {
      UINT c = _dist(e);
    ...
    template<typename ENGINE> BUN_FORCEINLINE UINT operator()(ENGINE& e) const { return Get<ENGINE>(e); }
