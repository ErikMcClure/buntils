...
namespace bun {
  ...
  BUN_FORCEINLINE static constexpr size_t AlignSize(size_t sz, size_t align)
  {
    return ((sz / align) + ((sz % align) != 0)) * align;
  }

  // An implementation of a standard allocator, with optional alignment
  template<typename T, int ALIGN = 0> class BUN_COMPILER_DLLEXPORT StandardAllocator
  {
#ifdef BUN_DEBUG
    union SIZETRACK
    {
  ...
  public:
    ...
    {
      typedef StandardAllocator<U, ALIGN> other;
    };
  ...
  template<typename T> struct BUN_COMPILER_DLLEXPORT AlignedAllocator : StandardAllocator<T, alignof(T)>
  {
  ...
  template<typename T> struct BUN_COMPILER_DLLEXPORT NullAllocator
  {
    using value_type = T;
  ...
  template<typename T, template<typename> class Policy>
    requires PolicyProvider<Policy, T>
  ...
  public:
    ...
    template<class U> struct rebind
    {
      typedef PolicyAllocator<U, Policy> other;
    };
    ...
    value_type* allocate(std::size_t n) { return _policy->allocate(n, nullptr, 0); }

    inline value_type* reallocate(size_t n, T* p, size_t oldsize) { return _policy->allocate(n, p, oldsize); }

    void deallocate(value_type* p, std::size_t sz) noexcept { _policy->deallocate(p, sz); }

    inline policy_type* move_policy() && noexcept
    {
      auto p  = _policy;
      _policy = nullptr;
      return p;
  ...
  template<class A>
    requires std::is_trivially_copyable_v<typename A::value_type>
  inline std::span<typename A::value_type> standard_realloc(A& allocator, size_t n, std::span<typename A::value_type> old)
  {
    ...
    constexpr bool has_realloc =
      requires(const A& a, int n, T* oldptr, size_t oldsize) { a.reallocate(n, oldptr, oldsize); };
