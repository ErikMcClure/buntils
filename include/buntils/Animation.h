...
namespace bun {
  ...
  class BUN_COMPILER_DLLEXPORT AniState : public AniStateBase
  {
  public:
  ...
  protected:
    template<size_t ...S> BUN_FORCEINLINE void _interpolate(std::index_sequence<S...>) { (std::get<S>(_states).template Interpolate<A, T>(_ani, _dest, _time), ...); }
    template<size_t ...S> BUN_FORCEINLINE void _reset(std::index_sequence<S...>) { (std::get<S>(_states).template Reset<T>(_dest), ...); }
    template<int I, typename X, typename... Xs>
    BUN_FORCEINLINE void _getvalues(VALUES& v) const
    {
    ...
    template<int I, typename X, typename... Xs>
    BUN_FORCEINLINE void _setvalues(const VALUES& v)
    {
      if constexpr(sizeof...(Xs) > 0 && std::is_same<typename X::VALUE, void>::value)
        _setvalues<I, Xs...>(v);
      else if constexpr(!std::is_same<typename X::VALUE, void>::value)
...
#pragma warning(pop)
  ...
  struct BUN_COMPILER_DLLEXPORT AniStateDiscrete
  {
  ...
  template<typename T, typename D, typename REF, void(T::*FN)(REF)>
  struct BUN_COMPILER_DLLEXPORT AniStateSmooth
  {
    using FRAME = typename D::FRAME;
...
#pragma warning(disable:4251)
  ...
  class BUN_COMPILER_DLLEXPORT Animation : public AnimationBase, public std::tuple<Args...>
  {
#pragma warning(pop)
  public:
    Animation() {}
  ...
  protected:
    template<size_t ...S> BUN_FORCEINLINE double _calclength(std::index_sequence<S...>) const { return std::max({ (std::get<S>(*this).End())... }); }
