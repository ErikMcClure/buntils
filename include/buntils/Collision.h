...
namespace bun {
  ...
  template<class T>
  BUN_FORCEINLINE int CircleCircleIntersect(T X1, T Y1, T R1, T X2, T Y2, T R2, Vector<T, 2>(&out)[2])
  {
  ...
  template<class T>
  BUN_FORCEINLINE bool CircleContainsPoint(T x, T y, T r, T pX, T pY)
  {
  ...
  template<class T>
  BUN_FORCEINLINE bool CircleContainsCircle(T X1, T Y1, T R1, T X2, T Y2, T R2)
  {
    if(R1 <= R2)
      return false;
  ...
  template<class T>
  BUN_FORCEINLINE bool CircleCircleCollide(T X1, T Y1, T R1, T X2, T Y2, T R2)
  {
    return RadiusContainsPoint(R1 + R2, X2 - X1, Y2 - Y1);
  }
  ...
  template<class T>
  BUN_FORCEINLINE void CircleNearestPoint(T x, T y, T r, T X, T Y, T& outX, T& outY) { RadiusNearestPoint(r, X - x, Y - y, outX, outY); outX += x; outY += y; }
  ...
  template<typename T>
  BUN_FORCEINLINE T LineSegmentPointDistance(T X1, T Y1, T X2, T Y2, T pX, T pY)
  {
  ...
  template<typename T>
  BUN_FORCEINLINE bool RectContainsPoint(T left, T top, T right, T bottom, T X, T Y)
  {
  ...
  template<typename T>
  BUN_FORCEINLINE bool EllipseContainsPoint(T X, T Y, T A, T B, T x, T y) 
  { 
    T tx = X - x; 
    T ty = Y - y; 
  ...
  template<typename T>
  BUN_FORCEINLINE bool EllipseCircleCollide(T X, T Y, T A, T B, T x, T y, T r)
  {
  ...
  template<typename T>
  BUN_FORCEINLINE bool EllipseRectCollide(T X, T Y, T A, T B, T left, T top, T right, T bottom)
  {
    T s = A / B;
  ...
  template<typename T>
  BUN_FORCEINLINE bool RadiusSectorContainsPointRadius(T X, T Y, T inner, T outer)
  {
    T dist = X*X + Y*Y;
  ...
  template<typename T>
  BUN_FORCEINLINE bool RadiusSectorContainsPointAngle(T X, T Y, T min, T range)
  {
    if(range >= (T)bun::PI_DOUBLE)
      return true;
  ...
  template<typename T>
  BUN_FORCEINLINE bool RadiusSectorContainsPoint(T X, T Y, T inner, T outer, T min, T range)
  {
    return RadiusSectorContainsPointRadius(X, Y, inner, outer) && RadiusSectorContainsPointAngle(X, Y, min, range);
  }
  template<typename T>
  BUN_FORCEINLINE bool CircleSectorContainsPoint(T x, T y, T inner, T outer, T min, T range, T X, T Y)
  {
