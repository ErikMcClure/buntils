...
namespace bun {
  ...
  public:
    ...
    {
      SetLength(n);
      MEMCPY(begin(), size() * sizeof(T), p, n * sizeof(T));
    }
...
#ifdef BUN_DEBUG
      if(length > size())
        bun_FillN<T>(end(), length - size(), 0xfd);
#endif
    ...
    BUN_FORCEINLINE const T& Front() const { assert(size() > 0); return begin()[0]; }
    BUN_FORCEINLINE const T& Back() const { assert(size() > 0); return begin()[size() - 1]; }
    BUN_FORCEINLINE T& Front() { assert(size() > 0); return begin()[0]; }
    BUN_FORCEINLINE T& Back() { assert(size() > 0); return begin()[size() - 1]; }
    BUN_FORCEINLINE const T* begin() const noexcept { return (_length&COMPACTFLAG) ? _internal : _array.data(); }
    BUN_FORCEINLINE const T* end() const noexcept { return begin() + size(); }
    BUN_FORCEINLINE T* begin() noexcept { return (_length&COMPACTFLAG) ? _internal : _array.data(); }
