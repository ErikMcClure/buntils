...
namespace bun {
  ...
  template<typename T, int O = 1> struct Dual
  {
    template<typename U> inline constexpr Dual(const Dual<U, O>& copy) : _x((T)copy._x), _dx(copy._dx) {}
    ...
    inline Dual operator/(const Dual& r) const
    {
      return Dual(_x / r._x, (_dx * r._low() - r._dx * _low()) / (r._low() * r._low()));
    }
    inline Dual operator^(double k) const
    {
      return Dual(std::pow(_x, (T)k), _dx * Dual<T, O - 1>(k) * (_low() ^ ((T)(k - 1))));
    }
    inline Dual operator^(const Dual& r) const
    {
      return Dual(std::pow(_x, r._x), (_low() ^ (r._low() - Dual<T, O - 1>(1))) *
                                        ((r._low() * _dx) + (_low() * r._dx * Dual<T, O - 1>::log(_low()))));
    }
    ...
    inline T operator()(int derivative) const
    {
      if(!derivative)
        return _x;
      return _dx(derivative - 1);
    }
    // inline operator T() const { return _x; }
    template<typename U> inline Dual& operator=(const Dual<U, O>& r)
    {
      _x  = (T)r._x;
      _dx = r._dx;
      return *this;
    ...
    inline static Dual log10(const Dual& d)
    {
      return Dual(std::log10(d._x), d._dx / (Dual<T, O - 1>(std::log((T)10)) * d._low()));
    }
  ...
  template<typename T> struct Dual<T, 0>
  {
...
template<typename T, int O> inline const bun::Dual<T, O> operator+(T l, const bun::Dual<T, O>& r)
{
  return bun::Dual<T, O>(l) + r;
}
template<typename T, int O> inline const bun::Dual<T, O> operator-(T l, const bun::Dual<T, O>& r)
{
  return bun::Dual<T, O>(l) - r;
