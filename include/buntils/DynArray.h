...
namespace bun {
  ...
  public:
    ...
    inline DynArray(const DynArray& copy) requires std::is_copy_constructible_v<RECURSIVE> : BASE(copy._array.size(), copy), _length(copy._length) { BASE::_copy(_array.data(), copy._array.data(), _length); }
    inline DynArray(DynArray&& mov) : BASE(std::move(mov)), _length(mov._length) { mov._length = 0; }
    inline explicit DynArray(std::span<const T> s) requires (std::is_copy_constructible_v<RECURSIVE> && std::is_default_constructible_v<Alloc>) : BASE(s.size()), _length(s.size()) { BASE::_copy(_array.data(), s.data(), s.size()); }
    ...
    inline ~DynArray() { BASE::_setLength(_array.data(), _length, 0); }
    BUN_FORCEINLINE CT Add(const T& item) { _checkSize(); new(_array.data() + _length) T(item); return _length++; }
    BUN_FORCEINLINE CT Add(T&& item) { _checkSize(); new(_array.data() + _length) T(std::move(item)); return _length++; }
    template<typename... Args>
    BUN_FORCEINLINE CT AddConstruct(Args&&... args) { _checkSize(); new(_array.data() + _length) T(std::forward<Args>(args)...); return _length++; }
    ...
    BUN_FORCEINLINE void Set(std::span<const T> slice) requires std::is_copy_constructible_v<RECURSIVE>
    {
      BASE::_setLength(_array.data(), _length, 0);
    ...
    {
      if(length < _length) BASE::_setLength(_array.data(), _length, length);
      if(length > _array.size()) BASE::_setCapacity(length, _length);
      if(length > _length) BASE::_setLength(_array.data(), _length, length);
      _length = length;
    }
    inline void SetCapacity(CT capacity) { if(capacity > _array.size()) BASE::_setCapacity(capacity, _length); }
    ...
    BUN_FORCEINLINE const T& operator[](CType i) const { assert(i < _array.size());  return _array[i]; }
    BUN_FORCEINLINE T& operator[](CType i) { assert(i < _array.size());  return _array[i]; }
    ...
    inline DynArray& operator=(Array<T, CT, Alloc>&& mov) { BASE::_setLength(_array.data(), _length, 0); BASE::operator=(std::move(mov)); _length = mov._array.size(); return *this; }
    ...
    inline DynArray& operator=(DynArray&& mov) { BASE::_setLength(_array.data(), _length, 0); BASE::operator=(std::move(mov)); _length = mov._length; mov._length = 0; return *this; }
  ...
  protected:
    ...
    BUN_FORCEINLINE void _checkSize()
    {
      if(_length >= _array.size())
        BASE::_setCapacity(T_FBNEXT(_array.size()), _length);
  ...
  public:
    inline DynArray(const DynArray& copy) : BASE(copy._array.size()), _length(copy._length) { memcpy(_array.data(), copy._array.data(), copy._array.size() * sizeof(STORE)); }
    inline DynArray(DynArray&& mov) : BASE(std::move(mov)), _length(mov._length) { mov._length = 0; }
    ...
    inline void SetLength(CT length)
    {
      if(length > (_array.size()*DIV_AMT)) BASE::_setCapacity(_maxChunks(length) / DIV_AMT);
      for(CT i = _length; i < length; ++i) GetBit(i) = false;
      _length = length;
    }
    inline void SetCapacity(CT capacity) { capacity = _maxChunks(capacity) / DIV_AMT; if(capacity > _array.size()) SetCapacity(capacity); }
    inline void Flip() { for(CT i = 0; i < _length; ++i) _array[i] = ~_array[i]; }
    ...
    BUN_FORCEINLINE CT Capacity() const { return _array.size()*DIV_AMT; }
    ...
    inline STORE GetRawByte(CT index) const { assert(index < _array.size()); return (index < (_array.size() - 1)) ? _array[index] : (_array[index] & ((STORE)(~0) >> ((_array.size()*DIV_AMT) - _length))); }
  ...
  public:
    inline ArbitraryArray(const ArbitraryArray& copy) : BASE(copy._array.size()), _length(copy._length), _element(copy._element) { memcpy(_array, copy._array, _length*_element); }
    ...
    {
      if((_length*_element) >= _array.size()) BASE::_setCapacity(T_FBNEXT(_length)*_element);
    ...
    inline void SetLength(CT length) { _length = length; length *= _element; if(length > _array.size()) BASE::_setCapacity(length); }
