...
namespace bun {
  ...
  inline float FixedPtConvf(T i) { static const T SMIN = bun_max(0, S - 24); return ((float)(i >> S)) + (((i&(((T)1 << S) - 1)) >> SMIN) / (float)((T)1 << (S - SMIN))); }
  template<typename T, T S>
  inline double FixedPtConvd(T i) { static const T SMIN = bun_max(0, S - 53); return ((double)(i >> S)) + (((i&(((T)1 << S) - 1)) >> SMIN) / (double)((T)1 << (S - SMIN))); }

  template<typename T, T S_X, T S_Y>
  inline T fixedpt_add(T x, T y) { return x + SafeShift<T, S_X - S_Y>(y); }
  template<typename T>
  inline T fixedpt_mul(T x, T y, T S_Y) { return bun_MultiplyExtract<T>(x, y, S_Y); } // The result has a scaling factor of 2^S_X * 2^S_Y = 2^(S_X + S_Y). To convert this back into 2^S_X, we divide by 2^S_Y, which is equivilent to shifting right by S_Y
