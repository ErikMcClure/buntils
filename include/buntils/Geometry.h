...
namespace bun {
  ...
  template<class T, int N> struct NSphere
  {
    template<typename U> inline constexpr NSphere(const NSphere<U, N>& copy) : c(copy.c), r(copy.r) {}
    ...
    template<class U> inline NSphere& operator=(const NSphere<U, N>& other)
    {
      c = other.c;
      r = other.r;
      return *this;
  ...
  template<class T> struct NSphere<T, 2>
  {
    template<typename U> inline constexpr NSphere(const NSphere<U, 2>& copy) : c(copy.c), r(copy.r) {}
    ...
    BUN_FORCEINLINE bool ContainsCircle(T X, T Y, T R) const
    {
      if(r <= R)
        return false;
      return RadiusContainsPoint(r - R, X - x, Y - y);
    }
    BUN_FORCEINLINE void NearestPoint(const Vector<T, 2>& p, T& outX, T& outY) const
    {
      RadiusNearestPoint(r, p.x - x, p.y - y, outX, outY);
    }
    BUN_FORCEINLINE void NearestPoint(T X, T Y, T& outX, T& outY) const { RadiusNearestPoint(r, X - x, Y - y, outX, outY); }
    BUN_FORCEINLINE int CircleIntersect(T X, T Y, T R, Vector<T, 2> (&out)[2]) const
    {
      return CircleRadiusIntersect<T>(r, X - x, Y - y, R, out);
    }
    ...
    {
      return LineRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r);
    }
    BUN_FORCEINLINE bool RayCollide(T X1, T Y1, T X2, T Y2) const
    {
      return RayRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r);
    }
    BUN_FORCEINLINE bool LineSegmentCollide(T X1, T Y1, T X2, T Y2) const
    {
      return LineSegmentRadiusCollide(X1 - x, Y1 - y, X2 - x, Y2 - y, r);
    }
    BUN_FORCEINLINE bool LineIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2> (&out)[2]) const
    {
      return LineRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out);
    }
    BUN_FORCEINLINE bool RayIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2> (&out)[2]) const
    {
      return RayRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out);
    }
    BUN_FORCEINLINE bool LineSegmentIntersect(T X1, T Y1, T X2, T Y2, Vector<T, 2> (&out)[2]) const
    {
      return LineSegmentRadiusIntersect(X1 - x, Y1 - y, X2 - x, Y2 - y, r, out);
    }
    BUN_FORCEINLINE bool RectCollide(T left, T top, T right, T bottom) const
    {
      return RectCircleCollide<T>(left, top, right, bottom, x, y, r);
    }
    BUN_FORCEINLINE bool EllipseCollide(T X, T Y, T A, T B) const { return EllipseCircleCollide(X, Y, A, B, x, y, r); }

    template<class U> inline NSphere& operator=(const NSphere<U, 2>& other)
    {
      c = other.c;
      r = other.r;
      return *this;
  ...
  template<class T> struct NSphere<T, 3>
  {
    template<typename U> inline constexpr NSphere(const NSphere<U, 3>& copy) : c(copy.c), r(copy.r) {}
    ...
    template<class U> inline NSphere& operator=(const NSphere<U, 3>& other)
    {
      c = other.c;
      r = other.r;
      return *this;
  ...
  template<class T, int N> struct LineN
  {
    using V = Vector<T, N>;
    inline constexpr LineN() {}
    template<class U> inline constexpr LineN(const LineN<U, N>& other) : p1(other.p1), p2(other.p2) {}
    ...
    template<class U> inline LineN& operator=(const LineN<U, N>& other)
    {
      p1 = other.p1;
      p2 = other.p2;
    }
  ...
  // 2D line
  template<class T> struct LineN<T, 2>
  {
    using V = Vector<T, 2>;
    inline constexpr LineN() {}
    ...
    template<class U> inline LineN& operator=(const LineN<U, 2>& other)
    {
      p1 = other.p1;
      p2 = other.p2;
    }
  ...
  // 3D line
  template<class T> struct LineN<T, 3>
  {
    using V = Vector<T, 3>;
    inline constexpr LineN() {}
    ...
    template<class U> inline LineN& operator=(const LineN<U, 3>& other)
    {
      p1 = other.p1;
      p2 = other.p2;
    }
  ...
  template<class T> struct Rect
  {
    ...
    template<class U>
    inline constexpr Rect(const Rect<U>& other) :
    ...
    inline explicit Rect(const T (&rectarray)[4]) :
      left(rectarray[0]), top(rectarray[1]), right(rectarray[2]), bottom(rectarray[3])
    {}
    inline explicit Rect(const std::array<T, 4>& rectarray) :
      left(rectarray[0]), top(rectarray[1]), right(rectarray[2]), bottom(rectarray[3])
    {}
    inline T Area() const { return (right - left) * (bottom - top); }
    inline Rect Abs() const { return Rect(abs(left), abs(top), abs(right), abs(bottom)); }
    inline Circle<T> BoundingCircle() const
    {
      V center = topleft + bottomright;
      center /= (T)2;
      return Circle<T>(center, Dist<T>(center.x, center.y, topleft.x, topleft.y));
    }
    inline V Center() const { return (topleft + bottomright) / (T)2; }
    BUN_FORCEINLINE V Dim() const { return V(right - left, bottom - top); }
    BUN_FORCEINLINE Rect Inflate(T amount) const
    {
    ...
    BUN_FORCEINLINE Rect Canonical() const
    {
      return Rect(bun_min(left, right), bun_min(top, bottom), bun_max(left, right), bun_max(top, bottom));
    }
    // Returns a new rectangle that includes the given point.
    BUN_FORCEINLINE Rect ExpandTo(const V& point) const
    {
      return Rect(bun_min(left, point.x), bun_min(top, point.y), bun_max(right, point.x), bun_max(bottom, point.y));
    }
    // Returns a rectangle that contains both this one and rect
    BUN_FORCEINLINE Rect Union(const Rect& rect) const
    {
      return Rect(bun_min(left, rect.left), bun_min(top, rect.top), bun_max(right, rect.right),
                  bun_max(bottom, rect.bottom));
    }
    // Returns a rectangle that contains only the overlapping parts (will be degenerate if there is no overlap)
    BUN_FORCEINLINE Rect Intersection(const Rect& rect) const
    {
      if(!RectCollide(rect))
        return Rect(left, top, left, top);
      return Rect(bun_max(left, rect.left), bun_max(top, rect.top), bun_min(right, rect.right),
                  bun_min(bottom, rect.bottom));
    }

    BUN_FORCEINLINE bool ContainsPoint(T X, T Y) const { return RectContainsPoint(left, top, right, bottom, X, Y); }
    BUN_FORCEINLINE bool ContainsRect(T Left, T Top, T Right, T Bottom) const
    {
      return RectContainsRect(left, top, right, bottom, Left, Top, Right, Bottom);
    }
    BUN_FORCEINLINE bool ContainsRect(const Rect& r) const
    {
      return RectContainsRect(left, top, right, bottom, r.left, r.top, r.right, r.bottom);
    }
    BUN_FORCEINLINE bool CircleCollide(T X, T Y, T R) const
    {
      return RectCircleCollide<T>(left, top, right, bottom, X, Y, R);
    }
    BUN_FORCEINLINE bool CircleCollide(const V& c, T R) const
    {
      return RectCircleCollide<T>(left, top, right, bottom, c.x, c.y, R);
    }
    BUN_FORCEINLINE bool CircleCollide(const Circle<T>& c) const
    {
      return RectCircleCollide<T>(left, top, right, bottom, c.x, c.y, c.r);
    }
    BUN_FORCEINLINE bool LineCollide(T X1, T Y1, T X2, T Y2) const
    {
      return RectLineCollide<T>(left, top, right, bottom, X1, Y1, X2, Y2);
    }
    BUN_FORCEINLINE bool LineCollide(const V& p1, const V& p2) const
    {
      return RectLineCollide<T>(left, top, right, bottom, p1.x, p1.y, p2.x, p2.y);
    }
    BUN_FORCEINLINE bool LineCollide(const Line<T>& l) const
    {
      return RectLineCollide<T>(left, top, right, bottom, l.x1, l.y1, l.x2, l.y2);
    }
    BUN_FORCEINLINE bool RectCollide(T Left, T Top, T Right, T Bottom) const
    {
      return RectRectCollide<T>(left, top, right, bottom, Left, Top, Right, Bottom);
    }
    BUN_FORCEINLINE bool RectCollide(const Rect& r) const
    {
      return RectRectCollide<T>(left, top, right, bottom, r.left, r.top, r.right, r.bottom);
    }
    ...
    inline Rect operator+(const V& other) const
    {
      return Rect(left + other.x, top + other.y, right + other.x, bottom + other.y);
    }
    inline Rect operator-(const V& other) const
    {
      return Rect(left - other.x, top - other.y, right - other.x, bottom - other.y);
    ...
    inline Rect& operator+=(const V& other)
    {
      left += other.x;
      top += other.y;
      right += other.x;
    ...
    inline Rect& operator+=(const T other)
    {
      left += other;
      top += other;
      right += other;
    ...
    {
      return (left != other) || (top != other) || (right != other) || (bottom != other);
    }
    inline bool operator==(const T other) const
    {
      return (left == other) && (top == other) && (right == other) && (bottom == other);
    ...
    inline bool operator>(const Rect& other) const
    {
      return (left > other.left) ||
             ((left == other.left) &&
              ((top > other.top) ||
               ((top == other.top) && ((right > other.right) || ((right == other.right) && (bottom > other.bottom))))));
    }
    inline bool operator<(const Rect& other) const
    {
      return (left < other.left) ||
    ...
    inline Rect& operator=(const sseVecT<T>& _right)
    {
      _right.Set(ltrb);
      return *this;
    }
    inline Rect& operator=(const Rect& _right)
    {
      left   = _right.left;
      top    = _right.top;
      right  = _right.right;
    ...
    template<class U> inline Rect& operator=(const Rect<U>& _right)
    {
      left   = (T)_right.left;
      top    = (T)_right.top;
      right  = (T)_right.right;
      bottom = (T)_right.bottom;
      return *this;
    }

    using SerializerArray = T;
    template<typename Engine> void Serialize(Serializer<Engine>& s, const char* id) { s.EvaluateFixedArray(ltrb, id); }
  ...
  template<class T> inline void RectExpandTo(Rect<T>& r, const Vector<T, 2>& p)
  {
    if(p.x < r.left)
      r.left = p.x;
    else if(p.x > r.right)
  ...
  template<class T> struct Triangle
  {
    ...
    template<class U> inline constexpr Triangle(const Triangle<U>& other)
    {
      for(int i = 0; i < 3; ++i)
        v[i] = other.v[i];
    }
  ...
  template<class T> struct Ellipse
  {
    ...
    template<class U> inline constexpr Ellipse(const Ellipse<U>& other) : pos(other.pos), axes(other.axes) {}
    template<class U> inline explicit constexpr Ellipse(const Circle<U>& other) : pos(other.pos), axes(other.r) {}
    ...
    BUN_FORCEINLINE void NearestPoint(T X, T Y, T& outX, T& outY) const
    {
      return EllipseNearestPoint(a, b, X - x, Y - y, outX, outY);
    }
    BUN_FORCEINLINE bool RectCollide(T left, T top, T right, T bottom) const
    {
      return EllipseRectCollide(x, y, a, b, left, top, right, bottom);
    }
    BUN_FORCEINLINE bool RectCollide(const Rect<T>& r) const
    {
      return EllipseRectCollide(x, y, a, b, r.left, r.top, r.right, r.bottom);
    }
  ...
  // 2D Circle Sector
  template<class T> struct CircleSector
  {
    using V = Vector<T, 2>;
    template<class U>
    inline constexpr CircleSector(const CircleSector<U>& other) :
      x((T)other.x), y((T)other.y), inner((T)other.inner), outer((T)other.outer), min((T)other.min), range((T)other.range)
    {}
    inline constexpr CircleSector(T X, T Y, T Inner, T Outer, T Min, T Range) :
      x(X), y(Y), inner(Inner), outer(Outer), min(bun_FMod<T>(Min, (T)PI_DOUBLE)), range(Range)
    {}
    inline constexpr CircleSector(const V& Pos, T Inner, T Outer, T Min, T Range) :
      x(Pos.x), y(Pos.y), inner(Inner), outer(Outer), min(bun_FMod<T>(Min, (T)PI_DOUBLE)), range(Range)
    {}
    ...
    inline bool CircleSectorCollide(T X, T Y, T Inner, T Outer, T Min, T Range)
    {
      ...
      if(fSmall(X) &&
         fSmall(Y)) // If this true, both the angle range and radius range must intersect for the segments to intersect
      ...
      if(inner > 0 &&
         _checkBothAngles(X, Y, CircleRadiusIntersect<T>(inner, X, Y, Outer, out), out, min, range, Min, Range))
      ...
      if(inner > 0 && Inner > 0 &&
         _checkBothAngles(X, Y, CircleRadiusIntersect<T>(inner, X, Y, Inner, out), out, min, range, Min, Range))
    ...
    {
      return LineSegmentLineSegmentCollide(cos(angle) * inner, -sin(angle) * inner, cos(angle) * outer, -sin(angle) * outer,
                                           X1, Y1, X2, Y2);
    }
    BUN_FORCEINLINE static bool _rayRayIntersect(T X, T Y, T Inner, T Outer, T Angle, T inner, T outer, T angle)
    {
      return _lineRayIntersect(X + cos(Angle) * Inner, Y - sin(Angle) * Inner, X + cos(Angle) * Outer,
                               Y - sin(Angle) * Outer, inner, outer, angle);
    }
    BUN_FORCEINLINE static bool _intersectRayCircle(T X, T Y, T R, T inner, T outer, T angle, T Min, T Range)
    {
      ...
      T c = cos(angle);
      T s = sin(angle);
      return _checkAngles(LineSegmentRadiusIntersect((c * inner) - X, -(s * inner) - Y, (c * outer) - X, -(s * outer) - Y,
                                                     R, out),
                          out, Min, Range);
    }
    ...
    static BUN_FORCEINLINE bool _checkBothAngles(T X, T Y, int n, const Vector<T, 2> (&in)[2], T min, T range, T Min,
                                                 T Range)
    {
      for(int i = 0; i < n; ++i)
        if(RadiusSectorContainsPointAngle(in[i].x, in[i].y, min, range) &&
           RadiusSectorContainsPointAngle(in[i].x - X, in[i].y - Y, Min, Range))
          return true;
      return false;
    }
  ...
  template<class T> inline Rect<T> PolygonAABB(const Vector<T, 2>* verts, size_t num)
  {
  ...
  template<class T> struct Polygon
  {
    ...
    inline Polygon() {}
    template<class U> inline Polygon(const Polygon<U>& other) : _verts(other._verts) {}
    inline Polygon(std::span<V> vertices) : _verts(vertices) {}
    template<class U> inline Polygon(std::span<Vector<U, 2>> vertices) : _verts(vertices) {}
    inline T Area() const { return PolygonArea<T>(_verts); }
    inline V Centroid() const { return PolygonCentroid<T>(_verts); }
    inline Rect<T> AABB() const { return PolygonAABB<T>(_verts); }
    inline bool IsConvex() const { return PolygonIsConvex<T>(_verts); }

    inline bool ContainsPoint(const Vector<T, 2>& point) const { return PolygonContainsPoint<T>(point, _verts); }
    inline bool ConvexContainsPoint(const Vector<T, 2>& point) const
    {
      return ConvexPolygonContainsPoint<T>(point, _verts);
    }
    ...
    inline Polygon<T>& operator=(const Polygon<T>& right)
    {
      _verts = right._verts;
      return *this;
    }
    inline Polygon<T>& operator=(Polygon<T>&& right)
    {
      _verts = std::move(right._verts);
      return *this;
    }
    template<class U> inline Polygon<T>& operator=(const Polygon<U>& right)
    {
    ...
    using SerializerArray = T;
    template<typename Engine> void Serialize(Serializer<Engine>& s, const char* id) { _verts.Serialize(s, id); }
