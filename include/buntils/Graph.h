...
namespace bun {
  namespace internal {
    template<typename D> struct BUN_COMPILER_DLLEXPORT VoidData
    {
      D data;
    };
    template<> struct BUN_COMPILER_DLLEXPORT VoidData<void>
    {};
  }

  // DAG edge
  template<typename E, typename CT = uint16_t>
  struct BUN_COMPILER_DLLEXPORT Edge : LLBase<Edge<E, CT>>, internal::VoidData<E>
  {
    CT to;
    CT from;
    LLBase<Edge<E, CT>> alt;
  };

  // Node for a DAG.
  template<typename E, typename V, typename CT = uint16_t> struct BUN_COMPILER_DLLEXPORT Node : internal::VoidData<V>
  {
    Edge<E, CT>* to;
    Edge<E, CT>* from;
  };
  ...
  namespace internal {
    ...
    template<typename V, typename CT> struct __Graph__InternalVertex
    {
    ...
    template<typename CT> struct __Graph__InternalVertex<void, CT>
    {
  ...
  template<typename E, typename V, typename CT = uint16_t, typename Alloc = StandardAllocator<Edge<E, CT>>,
           typename NODEALLOC = StandardAllocator<internal::LINKEDNODE<Node<E, V, CT>, CT>>>
  class Graph :
    public Alloc,
    protected internal::__Graph__InternalEdge<E>,
    protected internal::__Graph__InternalVertex<V, CT>
  {
  ...
  public:
    inline Graph(Graph&& mov) : Alloc(std::move(mov)), _nodes(std::move(mov._nodes)), _nedges(mov._nedges)
    {
      mov._nedges = 0;
    }
    ...
    inline Graph(CT n, const char* M, const V* nodes)
      requires std::is_default_constructible_v<Alloc>
      : _nodes(n), _nedges(0)
    {
      _construct<char, _baseCheck>(n, M, nodes);
    }
    // Build a matrix out of only a list of nodes
    inline Graph(CT n, const V* nodes)
      requires std::is_default_constructible_v<Alloc>
      : _nodes(n), _nedges(0)
    {
      for(CT i = 0; i < n; ++i)
    ...
    inline typename LinkedArray<Node<E, V, CT>, CT, NODEALLOC>::template cLAIter<true> begin() const
    {
      return _nodes.begin();
    }
    ...
    inline CT AddNode()
    {
      Node<E, V, CT> aux;
      bun_Fill(aux, 0);
      return _nodes.Add(aux);
    }
    inline CT AddNode(const V* node)
    {
      if(!node)
        return AddNode();
      Node<E, V, CT> aux;
    ...
    template<bool ISEDGE(const E*)> void FromMatrix(CT n, const E* matrix, const V* nodes)
    {
      _construct<E, ISEDGE>(n, matrix, nodes);
    }
  ...
  protected:
    template<typename D, bool (*ISEDGE)(const D*)> void _construct(CT n, const D* M, const V* nodes)
    {
      VARARRAY(CT, k, n);
  ...
  namespace internal {
    template<typename E> struct BUN_COMPILER_DLLEXPORT __edge_MaxFlow
    {
      int flow;
      int capacity;
      VoidData<E> d;
      ...
      while(excess[k] > 0) // Discharge current vertex
      {
        if(!seen[k]) // If seen is null, we tried all our neighbors so relabel
        {
          ...
          { // First we try to send it forward
            ...
            {
              if(height[edge->to] < send)
                send = height[edge->to];
              height[k] =
                send + 1; // we have to do this in here because if there is no valid relabel, the height can't change.
            }
          }

          if(send ==
             INT_MAX) // If this is true, our forward relabel failed, so that means we need to try to push things backwards
          {
  ...
  namespace internal {
    template<typename V> struct BUN_COMPILER_DLLEXPORT __vertex_Demand
    {
      int demand;
      VoidData<V> d;
    };
  ...
  template<typename E, typename V, typename CT, typename ALLOC, typename NODEALLOC>
  inline int Circulation_MaxFlow(Graph<internal::__edge_MaxFlow<E>, internal::__vertex_Demand<V>, CT, ALLOC, NODEALLOC>& g,
                                 CT& s, CT& t)
  {
  ...
  namespace internal {
    template<typename E> struct BUN_COMPILER_DLLEXPORT __edge_LowerBound
    {
      int lowerbound;
      VoidData<E> d;
    };
  ...
  template<typename E, typename V, typename CT, typename ALLOC, typename NODEALLOC>
  inline void LowerBound_Circulation(
    Graph<internal::__edge_MaxFlow<internal::__edge_LowerBound<E>>, internal::__vertex_Demand<V>, CT, ALLOC, NODEALLOC>& g)
  {
  ...
  template<typename G, bool (*FACTION)(typename G::CT_)> inline void BreadthFirstGraph(G& graph, typename G::CT_ root)
  {
    VARARRAY(typename G::CT_, queue, graph.NumNodes());
    BreadthFirstGraph<G, FACTION>(graph, root, queue);
  }

  // Breadth-first search for any directed graph. If FACTION returns true, terminates. queue must point to an array at least
  // GetNodes() long.
