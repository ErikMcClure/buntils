...
namespace bun {
  ...
  class BUN_DLLEXPORT INIsection
  {
  public:
    ...
    BUN_FORCEINLINE INIstorage* GetParent() const { return _parent; }
    BUN_FORCEINLINE const char* GetName() const { return _name; }
    BUN_FORCEINLINE size_t GetIndex() const { return _index; }

    BUN_FORCEINLINE INIentry& operator[](const char* key) const { return GetEntry(key, 0); }
