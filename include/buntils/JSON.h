...
namespace bun {
  ...
  public:
    ...
    template<typename T> static void Serialize(Serializer<JSONEngine>& e, const T& obj, const char* id);
    template<typename T> static void SerializeArray(Serializer<JSONEngine>& e, const T& obj, size_t size, const char* id);
    template<typename T, size_t... S>
    static void SerializeTuple(Serializer<JSONEngine>& e, const T& obj, const char* id, std::index_sequence<S...>);
    template<typename T> static void SerializeNumber(Serializer<JSONEngine>& e, T obj, const char* id);
    ...
    template<typename T> static void Parse(Serializer<JSONEngine>& e, T& obj, const char* id);
    template<typename T, typename E, void (*Add)(Serializer<JSONEngine>& e, T& obj, int& n),
             bool (*Read)(Serializer<JSONEngine>& e, T& obj, int64_t count)>
    static void ParseArray(Serializer<JSONEngine>& e, T& obj, const char* id);
    template<typename T> static void ParseNumber(Serializer<JSONEngine>& e, T& obj, const char* id);
    ...
    template<typename F> static void ParseMany(Serializer<JSONEngine>& e, F&& f);

    static inline void ParseJSONEatWhitespace(std::istream& s)
    {
    ...
    template<typename F> static inline void ParseJSONObject(Serializer<JSONEngine>& e, F f);
    ...
    static inline size_t WriteJSONPretty(size_t pretty)
    {
      return (pretty & (~JSONEngine::PRETTYFLAG)) + ((pretty & (~JSONEngine::PRETTYFLAG)) > 0);
    }
  ...
  struct JSONValue :
    public Variant<Str, double, int64_t, bool, DynArray<JSONValue, size_t, StandardAllocator<JSONValue>, std::monostate>,
                   DynArray<std::pair<Str, JSONValue>, size_t, StandardAllocator<std::pair<Str, JSONValue>>, std::monostate>>
  {
  ...
  public:
    ...
    template<typename T> explicit JSONValue(const T& obj) : BASE(obj) {}
    template<typename T>
    ...
    {
      BASE::operator=(internal::serializer::ConvRef<JSONValue>::Value<T, BASE>::f(right));
      return *this;
    }
  };

  template<typename F> void JSONEngine::ParseJSONObject(Serializer<JSONEngine>& e, F f)
  {
    std::istream& s = *e.in;
    Str buf;
    ...
    if(s.peek() == '}')
      s.get(); // eat the closing brace
  }

  template<class T> inline void ParseJSONBase(Serializer<JSONEngine>& e, T& obj, std::istream& s)
  {
    static_assert(internal::serializer::is_serializable<JSONEngine, T>::value,
                  "object missing Serialize<Engine>(Serializer<Engine>&, const char*) function!");
    obj.template Serialize<JSONEngine>(e, 0);
  }
  template<class T> inline void ParseJSON(T& obj, std::istream& s)
  {
    Serializer<JSONEngine> e;
    e.Parse<T>(obj, s, 0);
  }
  template<class T> inline void ParseJSON(T& obj, const char* s)
  {
    std::istringstream ss(s);
    ParseJSON<T>(obj, ss);
  }

  template<> inline void ParseJSONBase<std::string>(Serializer<JSONEngine>& e, std::string& target, std::istream& s)
  {
    target.clear();
  ...
  template<> inline void ParseJSONBase<Str>(Serializer<JSONEngine>& e, Str& target, std::istream& s)
  {
    ParseJSONBase<std::string>(e, target, s);
  }

  template<> inline void ParseJSONBase<JSONValue>(Serializer<JSONEngine>& e, JSONValue& target, std::istream& s)
  {
    ...
    default:
    {
      Str buf;
      ParseJSONBase<Str>(e, buf, s);
      if(!STRICMP(buf.c_str(), "true"))
        target = true;
      else if(!STRICMP(buf.c_str(), "false"))
        target = false;
      else if(!STRICMP(buf.c_str(), "null"))
        break;
      else
        target = buf;
      break;
  ...
  template<typename T> void JSONEngine::Parse(Serializer<JSONEngine>& e, T& obj, const char* id)
  {
    ParseJSONBase<T>(e, obj, *e.in);
  }
  template<typename F> void JSONEngine::ParseMany(Serializer<JSONEngine>& e, F&& f) { ParseJSONObject<F>(e, f); }
  template<typename T, typename E, void (*Add)(Serializer<JSONEngine>& e, T& obj, int& n),
           bool (*Read)(Serializer<JSONEngine>& e, T& obj, int64_t count)>
  ...
  template<typename T> void JSONEngine::ParseNumber(Serializer<JSONEngine>& e, T& obj, const char* id)
  {
    std::istream& s = *e.in;
    if(s.peek() == ',' || s.peek() == ']' || s.peek() == '}')
      return;
  ...
  // To enable pretty output, set pretty to 1. The upper two bits are used as flags, so if you need more than 1073741823
  // levels of indentation... what the hell are you doing?!
  template<class T> inline void WriteJSONBase(Serializer<JSONEngine>& e, const char* id, const T& obj, std::ostream& s)
  {
    JSONEngine::WriteJSONObject<T>(e, id, obj, s, e.engine.pretty);
  }
  ...
  template<> inline void WriteJSONBase<Str>(Serializer<JSONEngine>& e, const char* id, const Str& obj, std::ostream& s)
  {
    WriteJSONBase<std::string>(e, id, obj, s);
  }

  template<class T> inline void WriteJSON(const T& obj, std::ostream& s, size_t pretty = 0)
  {
    Serializer<JSONEngine> e;
    e.engine.pretty = pretty;
    e.Serialize<T>(obj, s, 0);
  }

  template<class T> inline void WriteJSON(const T& obj, const char* file, size_t pretty = 0)
  {
    std::ofstream fs(file, std::ios_base::out | std::ios_base::trunc | std::ios_base::binary);
    WriteJSON<T>(obj, fs, pretty);
  }
  ...
  inline void WriteJSONBase<JSONValue>(Serializer<JSONEngine>& e, const char* id, const JSONValue& obj, std::ostream& s)
  {
    switch(obj.tag())
    {
    case JSONValue::Type<Str>::value:
      Serializer<JSONEngine>::template ActionBind<Str>::Serialize(e, obj.get<Str>(), id);
      break;
    case JSONValue::Type<double>::value:
      Serializer<JSONEngine>::template ActionBind<double>::Serialize(e, obj.get<double>(), id);
      break;
    case JSONValue::Type<int64_t>::value:
      Serializer<JSONEngine>::template ActionBind<int64_t>::Serialize(e, obj.get<int64_t>(), id);
      break;
    case JSONValue::Type<bool>::value:
      Serializer<JSONEngine>::template ActionBind<bool>::Serialize(e, obj.get<bool>(), id);
      break;
    ...
    case JSONValue::Type<JSONValue::JSONObject>::value:
      Serializer<JSONEngine>::template ActionBind<JSONValue::JSONObject>::Serialize(e, obj.get<JSONValue::JSONObject>(),
                                                                                    id);
      break;
    }
  }
  template<typename T> void JSONEngine::SerializeArray(Serializer<JSONEngine>& e, const T& obj, size_t size, const char* id)
  {
    std::ostream& s = *e.out;
  ...
  template<typename T> void JSONEngine::SerializeNumber(Serializer<JSONEngine>& e, T obj, const char* id)
  {
    WriteJSONComma(*e.out, e.engine.pretty);
    WriteJSONId(id, *e.out, e.engine.pretty);
    (*e.out) << obj;
  ...
  template<typename T> void JSONEngine::Serialize(Serializer<JSONEngine>& e, const T& obj, const char* id)
  {
