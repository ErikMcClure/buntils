...
namespace bun {
  ...
  template<typename T> struct KDNode
  {
    ...
    float total[2]; // holds the true accumulations. This may cause precision problems, but calculating and updating the
                    // true average is more expensive
  };

  // KD-tree storing arbitrary rectangles. Requires a function turning T into a float[4] reference, LLBASE<T> list, and an
  // action.
  template<typename T, const float* (*FRECT)(T*), LLBase<T>& (*FLIST)(T*), KDNode<T>*& (*FNODE)(T*),
           typename Alloc = PolicyAllocator<KDNode<T>, BlockPolicy>>
  ...
  public:
    ...
    template<void (*F)(T*)> void Traverse(float (&rect)[4]) const
    {
      if(_root)
        _traverse<0, 1, F>(_root, rect);
    }
    template<typename F> void TraverseAction(float (&rect)[4], F&& f) const
    {
      if(_root)
        _traverseAction<0, 1, F>(_root, rect, std::forward<F>(f));
    }
    template<typename F> void TraverseAll(F&& f) { _traverseAll(_root, std::forward<F>(f)); }
    void Insert(T* item)
    {
      KDNode<T>** p = &_root;
      KDNode<T>* h;
    ...
    BUN_FORCEINLINE void Rebalance(KDNode<T>* node)
    {
      ...
      _rebalance(
        node, rect, parents,
        total); // We don't bother trying to do a checkdestroy here because it isn't possible. In order for this node to be
                // rebalanced, it must have images, and those images cannot go higher than this node in the tree.
    }
  ...
  protected:
    void _solve(KDNode<T>** pnode)
    {
      ...
      while((item = next))
      {
        ...
        if(r[node->axis + 2] < node->div)
          p = &node->left; // Drop into left
        else if(r[node->axis] > node->div)
          p = &node->right; // Drop into right
        else
          continue;
    ...
    void _rebalance(KDNode<T>* node, const float (&r)[4], KDNode<T>* const* p, float (&total)[2])
    {
      ...
      float rect[4] = {
        r[0], r[1], r[2], r[3]
      }; // Run down through the tree, resetting all the divider lines to whatever the current total is.
      ...
      while((item = next) != nullptr)
      {
        ...
        if(itemr[0] < r[0])
          i = 0; // Check for a violation. We don't bother checking to see if there are multiple violations, because
        if(itemr[1] < r[1])
          i = 1; // if that happens, we'll catch it on our way back up the stack.
        if(itemr[2] > r[2])
          i = 2;
        if(itemr[3] > r[3])
          i = 3;
        ...
        par = p[i];
        par->total[0] +=
          itemr[0] + itemr[2]; // Add image to violated parent. We don't worry about the middle, because the rebalance will
    ...
    template<typename F> static void _traverseAll(KDNode<T>* node, const F& f)
    {
      if(!node)
        return;
    ...
    template<char cur, char next, void (*FACTION)(T*)> static void _traverse(const KDNode<T>* node, const float (&rect)[4])
    {
      T* item = node->items;
      const float* r;
    ...
    inline void _removeItem(KDNode<T>* node, T* item)
    {
      ...
      LLBase<T>& l = FLIST(item);
      if(l.prev)
        FLIST(l.prev).next = l.next;
      else
        node->items = l.next;
      if(l.next)
        FLIST(l.next).prev = l.prev;
    }
