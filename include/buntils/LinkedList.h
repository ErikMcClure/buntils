...
namespace bun {
  ...
  template<typename T> struct LLNode : LLBase<LLNode<T>>
  {
    T item;
  };

  // Generic Iterator for LinkedList
  template<typename T> class BUN_COMPILER_DLLEXPORT LLIter
  {
  ...
  public:
    ...
    inline LLIter& operator++()
    {
      cur = cur->next;
      return *this;
    } // prefix
    inline LLIter operator++(int)
    {
      LLIter r = *this;
      ++*this;
      return r;
  ...
  namespace internal {
    ...
    template<> struct LList_SIZE<false>
    {
      inline void _incsize() {}
      inline void _decsize() {}
      inline void _zerosize() {}
    };

    // Adaptive class template for _last usage
    template<typename T, bool L> struct LList_LAST
    {
      ...
      { // if target is 0 we start from the root
        ...
        while(cur->item != item)
        {
          cur = backwards ?
                  cur->prev :
                  cur->next; // This method of finding an object is such bad practice we just let the CPU optimize this out.
          if(!cur)
            cur = rootflip; // return to the flipside value
          if(cur == from)
            return 0; // if we have returned to the starting node, the item doesn't exist
        }
        return cur;
      }
    ...
    protected:
      ...
      inline LList_LAST(LList_LAST<T, L>&& mov) : _last(mov._last), _root(mov._root)
      {
        mov._root = 0;
        mov._last = 0;
      }
      inline void _add(LLNode<T>* node)
      {
        if(!_root)
        {
          ...
          _last = LLAddAfter<LLNode<T>>(node, _last);
      }
      inline void _remove(LLNode<T>* node) { LLRemove<LLNode<T>>(node, _root, _last); }
      inline LList_LAST<T, L>& operator=(LList_LAST<T, L>&& mov)
      {
        _last     = mov._last;
        mov._last = 0;
        _root     = mov._root;
    ...
    template<typename T> struct LList_LAST<T, false>
    {
      ...
      { // if target is 0 we start from the root
        ...
        while(cur->item != item)
        {
          ...
          if(!cur)
            cur = _root; // return to the flipside value
          if(cur == from)
            return 0; // if we have returned to the starting node, the item doesn't exist
        }
        return cur;
    ...
    protected:
      ...
      inline LList_LAST<T, false>& operator=(LList_LAST<T, false>&& mov)
      {
        _root     = mov._root;
        mov._root = 0;
        return *this;
  ...
  class BUN_COMPILER_DLLEXPORT LinkedList :
    protected Alloc,
    public internal::LList_SIZE<useSize>,
    public internal::LList_LAST<T, useLast>
  {
  protected:
  ...
  public:
    ...
    inline LLIter<const T> begin() const
    {
      return LLIter<const T>(_root);
    ...
    template<typename U, bool V> inline LinkedList& operator=(const LinkedList<T, U, useLast, V>& right)
    {
      if(&right == this)
        return *this;
      Clear();
    ...
    template<typename U, bool V> inline LinkedList& operator=(LinkedList<T, U, useLast, V>&& mov)
    {
      if(&mov == this)
        return *this;
    ...
    template<typename U, bool L, bool V> inline LinkedList& operator+=(const LinkedList<T, U, L, V>& right)
    {
