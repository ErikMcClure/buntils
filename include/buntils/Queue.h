...
namespace bun {
  // Fast, tiny circular array-based queue. Pop and Peek are only valid if there is an item in the stack; this check must be
  // done by the user.
  ...
  public:
    ...
    BUN_FORCEINLINE T& Peek()
    {
      assert(_length != 0);
      return BASE::Back();
    }
    BUN_FORCEINLINE const T& Peek() const
    {
      assert(_length != 0);
      return BASE::Back();
    }
  ...
  protected:
    template<typename U> void _push(U&& value)
    {
      if(_length >= Capacity())
