...
namespace bun {
  // Write-preferring Readers-Writer lock, although writer starvation is still theoretically possible if an infinite number
  // of new readers attempt to acquire the lock
  ...
  public:
    ...
    inline RWLock() :
      l(0) // Note: if required, the fetch_or behavior can be emulated by checking a write lock, incrementing the read lock,
           // then checking the write lock again.
    {
    ...
    BUN_FORCEINLINE void Lock() noexcept
    {
      ...
      while(l.fetch_or(WFLAG, std::memory_order_acquire) & WFLAG)
        ; // While the returned value includes the flag bit, another writer is performing an operation
      while((l.load(std::memory_order_acquire) & WMASK) > 0)
        ; // Wait for any remaining readers to flush
    }
    ...
    {
      // assert(debugEmplace());
      while(l.fetch_add(ONE_READER, std::memory_order_acquire) &
            WFLAG) // Oppurtunistically acquire a read lock and check to see if the writer flag is set
      {
        if(l.fetch_sub(ONE_READER, std::memory_order_release) &
           WFLAG) // If the writer flag is set, release our lock to let the writer through
          while(l.load(std::memory_order_acquire) & WFLAG)
            ; // Wait until the writer flag is no longer set before looping for another attempt
      }
    }
    ...
    {
      if(l.fetch_add(ONE_READER, std::memory_order_acquire) &
         WFLAG) // Oppurtunistically acquire a read lock and check to see if the writer flag is set
      {
    ...
    // Upgrades a read lock to a write lock without releasing the read lock. You CANNOT release this write lock using
    // Unlock(), you have to use Downgrade() followed by RUnlock().
    BUN_FORCEINLINE void Upgrade() noexcept
    {
      ...
      while(l.fetch_or(WFLAG, std::memory_order_acquire) & WFLAG) // Attempt to acquire the write lock
      {
        ...
        while(l.load(std::memory_order_acquire) & WFLAG)
          ; // Wait until the writer flag is no longer set before looping for another attempt
        l.fetch_add(ONE_READER, std::memory_order_acquire); // Acquire a read lock before our next upgrade attempt - if the
                                                            // attempt fails, we'll release this.
      }
      while((l.load(std::memory_order_acquire) & WMASK) > ONE_READER)
        ; // Only flush to a single read lock, which will be our own
    }
    ...
    BUN_FORCEINLINE void Downgrade() noexcept
    {
      // assert(debugCount() == 1);
      assert(l.load(std::memory_order_acquire) & WFLAG);
      l.fetch_and(WMASK, std::memory_order_release); // Even though this actually does the same thing as Unlock, we force
                                                     // you to call this function instead so the debug checks are valid.
    }

    BUN_FORCEINLINE size_t ReaderCount() noexcept { return l.load(std::memory_order_acquire) & WMASK; }
...
#pragma warning(disable : 4251)
    ...
    bool debugEmplace()
    {
      while(_debuglock.test_and_set(std::memory_order_acquire))
        ;
      bool r = _debug.emplace(std::this_thread::get_id(), 0).second;
      _debuglock.clear(std::memory_order_release);
      return r;
    }
    size_t debugErase()
    {
      while(_debuglock.test_and_set(std::memory_order_acquire))
        ;
      size_t r = _debug.erase(std::this_thread::get_id());
      _debuglock.clear(std::memory_order_release);
      return r;
    }
    size_t debugCount()
    {
      while(_debuglock.test_and_set(std::memory_order_acquire))
        ;
      size_t r = _debug.count(std::this_thread::get_id());
      _debuglock.clear(std::memory_order_release);
      return r;
    }
