...
namespace bun {
  ...
  public:
    ...
    template<class ENGINE> inline T Pop(ENGINE& e)
    {
    ...
    inline RandomQueue& operator+=(const RandomQueue& add)
    {
      BASE::operator+=(add);
      return *this;
    }
    inline const RandomQueue operator+(const RandomQueue& add) const
    {
      RandomQueue r(*this);
      return (r += add);
    }
  ...
  inline void PoissonDiskSample(T (&rect)[4], T mindist, F&& f, size_t pointsPerIteration = 30)
  {
    ...
    while(!list.Empty())
    {
      ...
      for(size_t i = 0; i < pointsPerIteration; i++)
      {
        ...
        {
          center =
            internal::_PDS_imageToGrid<T>(pt, cell, gw, rect); // If another point is in the neighborhood, abort this point.
          edge = center - gw - gw;
          assert(edge > 0);
#define POISSONSAMPLE_CHECK(edge)                                                         \
  if((~ig[edge]) != 0 && DistSqr(grid[edge][0], grid[edge][1], pt[0], pt[1]) < mindistsq) \
  continue
          POISSONSAMPLE_CHECK(edge - 1);
          POISSONSAMPLE_CHECK(edge);
          POISSONSAMPLE_CHECK(edge + 1);
          edge += gw;
          if(~(ig[edge - 1] & ig[edge] & ig[edge + 1]))
            continue;
