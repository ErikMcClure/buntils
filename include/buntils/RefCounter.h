...
#ifndef __REFCOUNTER_H__BUN__ // These are used in case this header file is used by two different projects dependent on each
...
namespace bun {
  ...
  protected:
    // Constructor - Reference is set to 0 because you may or may not have a persistent reference to this, or something else
    // will try to grab it or whatever
    ...
    inline RefCounter& operator=(const RefCounter& right)
    {
      return *this;
    } // This does not actually change the reference count
  ...
  template<class T> class BUN_COMPILER_DLLEXPORT ref_ptr
  {
  public:
    inline ref_ptr(const ref_ptr& copy) : _p(copy._p)
    {
      if(_p != nullptr)
        _p->Grab();
    }
    inline ref_ptr(ref_ptr&& mov) : _p(mov._p) { mov._p = nullptr; }
    inline ref_ptr(T* p = nullptr) : _p(p)
    {
      if(_p != nullptr)
        _p->Grab();
    }
