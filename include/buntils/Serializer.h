...
namespace std {
  template<class... X> // Override std::end for tuples so retrieving the last element of the fake tuple "array" is
                       // syntactically valid
  constexpr auto end(std::tuple<X...>& c) -> decltype(&std::get<sizeof...(X) - 1>(c))
  {
    return &std::get<sizeof...(X) - 1>(c);
  }
}

namespace bun {
  template<class T> std::pair<const char*, T&> GenPair(const char* l, T& r) { return std::pair<const char*, T&>(l, r); }

  namespace internal {
    namespace serializer {
      template<class T> struct PushValue
      {
      ...
      template<class E> struct ConvRef
      {
        template<class T, class BASE> struct Value
        {
          inline static constexpr T&& f(typename std::remove_reference<T>::type& r) { return (static_cast<T&&>(r)); }
          inline static constexpr T&& f(typename std::remove_reference<T>::type&& r) { return (static_cast<T&&>(r)); }
        };
        template<class BASE> struct Value<E, BASE>
        {
          inline static constexpr BASE&& f(typename std::remove_reference<E>::type& r) { return (static_cast<BASE&&>(r)); }
          inline static constexpr BASE&& f(typename std::remove_reference<E>::type&& r) { return (static_cast<BASE&&>(r)); }
        };
        template<class BASE> struct Value<const E, BASE>
        {
          inline static constexpr const BASE&& f(typename std::remove_reference<const E>::type& r)
          {
            return (static_cast<const BASE&&>(r));
          }
          inline static constexpr const BASE&& f(typename std::remove_reference<const E>::type&& r)
          {
            return (static_cast<const BASE&&>(r));
      ...
      {};
      template<class T>
      struct is_serializer_array<T, std::void_t<typename T::SerializerArray>> :
        std::bool_constant<!std::is_void_v<typename T::SerializerArray>>
      {};
      ...
      template<class Engine, class T> struct Action // Arbitrary value
      {
        static inline void Parse(Serializer<Engine>& e, T& obj, const char* id)
        {
          if constexpr(is_serializer_array<T>::value)
          {
            ...
            Engine::template ParseNumber<typename make_integral<T>::type>(
              e, reinterpret_cast<typename make_integral<T>::type&>(obj), id);
          else
        ...
        static inline void Serialize(Serializer<Engine>& e, const T& obj, const char* id)
        {
          if constexpr(is_serializer_array<T>::value)
          {
            ...
            Engine::template SerializeNumber<typename make_integral<T>::type>(
              e, reinterpret_cast<const typename make_integral<T>::type&>(obj), id);
          else
            Engine::template Serialize<T>(e, obj, id);
        }
      };

      template<class T> inline T FromString(const char* id)
      {
      ...
      {
        static inline void Parse(Serializer<Engine>& e, internal::_BIT_REF<STORE>& obj, const char* id)
        {
          bool b = obj;
          Engine::ParseBool(e, b, id);
          obj = b;
        }
        static inline void Serialize(Serializer<Engine>& e, internal::_BIT_REF<STORE> obj, const char* id)
        {
          Engine::SerializeBool(e, obj, id);
        }
      };
      ...
      {
        static inline void Parse(Serializer<Engine>& e, T (&obj)[I], const char* id)
        {
          Engine::template ParseArray<T[I], T, &Serializer<Engine>::template FixedAdd<T[I]>,
                                      &Serializer<Engine>::template FixedRead<T[I]>>(e, obj, id);
        }
        static inline void Serialize(Serializer<Engine>& e, const T (&obj)[I], const char* id)
        {
          Engine::template SerializeArray<T[I]>(e, obj, I, id);
        }
      };
      ...
      {
        static inline void Parse(Serializer<Engine>& e, std::array<T, I>& obj, const char* id)
        {
          Engine::template ParseArray<std::array<T, I>, T, &Serializer<Engine>::template FixedAdd<std::array<T, I>>,
                                      &Serializer<Engine>::template FixedRead<std::array<T, I>>>(e, obj, id);
        }
        static inline void Serialize(Serializer<Engine>& e, const std::array<T, I>& obj, const char* id)
        {
          Engine::template SerializeArray<std::array<T, I>>(e, obj, I, id);
        }
      };

      template<class Engine, class T, typename Alloc> struct Action<Engine, std::vector<T, Alloc>>
      {
      ...
      template<class Engine, class... Args> struct Action<Engine, std::tuple<Args...>>
      {
        template<int I> static inline void _add(Serializer<Engine>& e, std::tuple<Args...>& obj, int n)
        {
          if(I == n)
        ...
        static inline void Add(Serializer<Engine>& e, std::tuple<Args...>& obj, int& n)
        {
          _add<sizeof...(Args) - 1>(e, obj, n++);
        }
        ...
        {
          Engine::template ParseArray<std::tuple<Args...>, std::tuple_element_t<sizeof...(Args) - 1, std::tuple<Args...>>,
                                      &Add, &Read>(e, obj, id);
        }
      ...
      template<class Engine, typename... Args> struct Action<Engine, ActionVariantRef<Args...>>
      {
  ...
  // Defines a universal serializer using an arbitrary serialization engine for types that implement `template<typename
  // Engine> void Serialize(Serializer<Engine>& e)`
  ...
  public:
    ...
    template<typename T> void Serialize(const T& obj, std::ostream& s, const char* name = 0)
    {
      out = &s;
      in  = 0;
      Engine::Begin(*this);
    ...
    template<typename T> void Parse(T& obj, std::istream& s, const char* name = 0)
    {
      out = 0;
      in  = &s;
      Engine::Begin(*this);
    ...
    template<typename T, typename... Args> inline void EvaluateType(std::pair<const char*, Args&>... args)
    {
      ...
      if(in) // Parsing
      {
        ...
        else
        {
          ...
          Engine::ParseMany(*this, [&](Serializer<Engine>& e, const char* id) {
            Serializer<Engine>::template FindParse<Args...>(e, id, trie, tmp);
          });
        }
      }
    }
    ...
    inline void EvaluateKeyValue(T& obj, F&& insert)
    {
      if(out) // Serializing
      {
        ...
        for(auto begin = std::begin(obj); begin != end; ++begin)
          ActionBind<std::remove_cvref_t<std::tuple_element_t<1, std::remove_cvref_t<decltype(*begin)>>>>::Serialize(
            *this, std::get<1>(*begin), ToString(std::get<0>(*begin)));
      }
    ...
    template<typename T, int I> BUN_FORCEINLINE void EvaluateFixedArray(T (&obj)[I], const char* id)
    {
      if(out) // Serializing
    ...
    template<int I, typename... Args> struct _findparse
    {
      inline static void F(Serializer<Engine>& e, uint16_t index, const std::tuple<std::pair<const char*, Args&>...>& args)
      {
        if(index == I)
          ActionBind<std::remove_reference_t<typename std::tuple_element_t<
            I, std::tuple<std::pair<const char*, Args&>...>>::second_type>>::Parse(e, std::get<I>(args).second,
    ...
    template<typename... Args> struct _findparse<0, Args...>
    {
      inline static void F(Serializer<Engine>& e, uint16_t index, const std::tuple<std::pair<const char*, Args&>...>& args)
      {
        if(index == 0)
          ActionBind<std::remove_reference_t<typename std::tuple_element_t<
            0, std::tuple<std::pair<const char*, Args&>...>>::second_type>>::Parse(e, std::get<0>(args).second,
                                                                                   std::get<0>(args).first);
      }
    };

    /*template<int I, typename... Args>
    inline static void r_findparse(Serializer<Engine>& e, uint16_t index, const std::tuple<std::pair<const char*,
    Args&>...>& args)
    {
      if(index == I)
        return ActionBind<std::remove_reference_t<typename std::tuple_element_t<I, std::tuple<std::pair<const char*,
    Args&>...>>::second_type>>::Parse(e, std::get<I>(args).second, std::get<I>(args).first); if constexpr(I > 0)
    r_findparse<I
    - 1, Args...>(e, index, args);
    }*/
    template<typename... Args> // This function must be static due to some corner cases on certain parsers
    inline static void FindParse(Serializer<Engine>& e, const char* key, const Trie<uint16_t>& trie,
    ...
    static inline void FixedAdd(Serializer<Engine>& e, auto& obj, int& n)
    {
      if(n < (std::end(obj) - std::begin(obj)))
        Serializer<Engine>::template ActionBind<std::remove_cvref_t<decltype(obj[0])>>::Parse(e, obj[n++], 0);
    }

    static inline bool FixedRead(Serializer<Engine>& e, auto& obj, int64_t count)
    {
      return e.BulkRead(std::begin(obj), std::end(obj), count);
    }
    ...
    template<class T> inline bool BulkRead(T&& begin, T&& end, int64_t count)
    {
      using Element = std::remove_cvref_t<decltype(*begin)>;
      if constexpr(std::is_trivially_constructible_v<Element> &&
                   (std::is_base_of_v<std::random_access_iterator_tag, T> || std::is_array_v<T> || std::is_pointer_v<T>))
      {
        if(begin == end)
          return false;

        auto& ref = *begin;
        in->read(reinterpret_cast<char*>(&ref), bun_min(count, end - begin) * sizeof(Element));
        return true;
      }
      else
        return false;
    ...
    template<class T> inline bool BulkWrite(T&& begin, T&& end, int64_t count)
    {
      using Element = std::remove_cvref_t<decltype(*begin)>;
      if constexpr(std::is_trivially_constructible_v<Element> &&
                   (std::is_base_of_v<std::random_access_iterator_tag, T> || std::is_array_v<T> || std::is_pointer_v<T>))
      {
        ...
        {
          auto& ref = *begin;
          out->write(reinterpret_cast<const char*>(&ref), bun_min(count, end - begin) * sizeof(Element));
        }
        return true;
      }
      else
  ...
  namespace internal {
    namespace serializer { // This is used by Serializer, but std::declval requires a complete type, so we have to put the
                           // partial specialization below Serializer
      template<class E, class T>
      struct is_serializable<
        E, T, std::void_t<decltype(std::declval<T>().template Serialize<E>(std::declval<Serializer<E>&>(), ""))>> :
        std::true_type
      {};
    }
  }
  ...
  public:
    ...
    template<typename T> static void Serialize(Serializer<EmptyEngine>&, const T&, const char*) {}
    template<typename T> static void SerializeArray(Serializer<EmptyEngine>&, const T&, size_t, const char*) {}
    ...
    template<typename T> static void SerializeNumber(Serializer<EmptyEngine>&, T, const char*) {}
    static void SerializeBool(Serializer<EmptyEngine>&, bool, const char*) {}
    template<typename T> static void Parse(Serializer<EmptyEngine>&, T&, const char*) {}
    template<typename T, typename E, void (*Add)(Serializer<EmptyEngine>&, T&, int&),
             bool (*Read)(Serializer<EmptyEngine>&, T&, int64_t)>
    static void ParseArray(Serializer<EmptyEngine>&, T&, const char*)
    {}
    template<typename T> static void ParseNumber(Serializer<EmptyEngine>&, T&, const char*) {}
    static void ParseBool(Serializer<EmptyEngine>&, bool&, const char*) {}
    template<typename F> static void ParseMany(Serializer<EmptyEngine>& e, F&& f) { f(e, ""); }
  };
}
