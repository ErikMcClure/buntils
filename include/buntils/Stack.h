...
namespace bun {
  // Fast, tiny array-based stack. Pop and Top are only valid if there is an item in the stack; this check must be done by
  // the user.
  template<class T, typename CType = size_t, typename Alloc = StandardAllocator<T>> class BUN_COMPILER_DLLEXPORT Stack
  {
  public:
    ...
    BUN_FORCEINLINE T Pop()
    {
      T r(std::move(_array.Back()));
      _array.RemoveLast();
      return std::move(r);
    }
    ...
    // Sets how many items are on the stack. Is usually used to truncate the stack, but can also be used to extend it. The
    // stack will follow the same rules that govern extending it's base array, so in a simple array, the values will be
    // uninitialized.
    ...
    inline Stack& operator=(const Stack& copy)
    {
      _array = copy._array;
      return *this;
    }
    inline Stack& operator=(Stack&& mov)
    {
    ...
    {
      _array.template Serialize<Engine>(s, id);
    }
