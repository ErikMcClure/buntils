...
namespace bun {
  ...
  /* This is a static class that holds specializations for char and wchar_t string operations */
    ...
    public:
      ...
      static BUN_FORCEINLINE CHAR* STOK(CHAR* str, const CHAR* delim, CHAR** context) { return STRTOK(str, delim, context); }
      static BUN_FORCEINLINE int VPF(CHAR *dest, size_t size, const CHAR *format, va_list args) { return VSNPRINTF(dest, size, format, args); }
      static BUN_FORCEINLINE int VPCF(const CHAR* str, va_list args) { return VSCPRINTF(str, args); }
      static BUN_FORCEINLINE size_t CONV(const OTHER_C* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF16toUTF8(src, srclen, dest, len); }
      static BUN_FORCEINLINE size_t CONV2(const OTHER_C2* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF32toUTF8(src, srclen, dest, len); }
    ...
    public:
      ...
      static BUN_FORCEINLINE CHAR* STOK(CHAR* str, const CHAR* delim, CHAR** context) { return WCSTOK(str, delim, context); }
#ifdef BUN_COMPILER_GCC
  ...
  //}
#else
      static BUN_FORCEINLINE int VPF(CHAR *dest, size_t size, const CHAR *format, va_list args) { return VSNWPRINTF(dest, size, format, args); }
      static BUN_FORCEINLINE int VPCF(const CHAR* str, va_list args) { return VSCWPRINTF(str, args); }
#endif
      static BUN_FORCEINLINE size_t CONV(const OTHER_C* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF8toUTF16(src, srclen, dest, len); }
      static BUN_FORCEINLINE size_t CONV2(const OTHER_C2* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF32toUTF16(src, srclen, dest, len); }
    ...
    public:
      ...
      static BUN_FORCEINLINE const CHAR* SCHR(const CHAR* str, CHAR val) { while(*str && *str != val) ++str; return str; }
      static BUN_FORCEINLINE size_t SLEN(const CHAR* str) { const CHAR* i = str; while(*i) ++i; return (size_t)(i - str); }
      ...
      static BUN_FORCEINLINE size_t CONV(const OTHER_C* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF8toUTF32(src, srclen, dest, len); }
      static BUN_FORCEINLINE size_t CONV2(const OTHER_C2* src, ptrdiff_t srclen, CHAR* dest, size_t len) { return UTF16toUTF32(src, srclen, dest, len); }
...
#pragma warning(disable: 4251)
  ...
  class BUN_COMPILER_DLLEXPORT StrT : public std::basic_string<T, std::char_traits<T>, Alloc> //If the constructors aren't inlined, it causes heap corruption errors because the basic_string constructors are inlined
...
#pragma warning(pop)
  ...
  template<typename T>
  BUN_FORCEINLINE std::conditional_t<std::is_same_v<std::decay_t<T>, char*> || std::is_same_v<std::decay_t<T>, const char*> || std::is_same_v<T, Str>, const T&, Str> ToString(const T& v)
  { 
...
// This allows Str to inherit all the string operations
  ...
  {
    bun::StrT<_Elem, _Alloc> _Ans(std::char_traits<_Elem>::length(_Left) + _Right.size()); _Ans += _Left; return (_Ans += _Right);
  }
  template<class _Elem, class _Alloc> // return character + string
  inline bun::StrT<_Elem, _Alloc> operator+(const _Elem _Left, const bun::StrT<_Elem, _Alloc>& _Right)
  {
    bun::StrT<_Elem, _Alloc> _Ans(1 + _Right.size()); _Ans += _Left; return (_Ans += _Right);
  }
