...
#ifndef __STR_TABLE_H__BUN__
#define __STR_TABLE_H__BUN__
...
namespace bun {
  // Given a large array of strings (or a memory dump), assembles a single chunk of memory into a series of strings that can
  // be accessed by index instantly
  template<typename T, typename CT_ = size_t> class BUN_COMPILER_DLLEXPORT StringTable
  {
  public:
    ...
    template<CT_ N> inline StringTable(const T* (&strings)[N]) : _strings(0), _indices(0) { _construct(strings, N); }
    ...
    inline const T* GetString(CT_ index) const
    {
    ...
    inline void DumpToStream(std::ostream* stream)
    {
      stream->write((const char*)_strings.data(), _strings.Capacity() * sizeof(T));
    }
    ...
    inline StringTable operator+(const StringTable& right)
    {
      StringTable retval(*this);
      return retval += right;
    }
    inline StringTable& operator+=(const char* right)
    {
      AppendString(right);
      return *this;
    }
  ...
  protected:
    void _construct(const T* const* strings, CT_ size)
    {
      ...
      for(CT_ i = 0; i < size;
          ++i) // this will cause an infinite loop if someone is dumb enough to set CT_ to an unsigned type and set size
               // equal to -1. If you actually think you could make that mistake, stop using this class.
      {
      ...
      for(CT_ i = 0; i < size; ++i) // this loop does two things - it copies the strings over, and it sets the index values
                                    // to the correct ones while discarding intermediate lengths
      {
        hold        = _indices[i];
        _indices[i] = curlength;
        memcpy(_strings.data() + curlength, strings[i],
               hold * sizeof(T)); // we could recalculate strlen here but that's dangerous because it might overwrite memory
        curlength += hold;
      }
    }
