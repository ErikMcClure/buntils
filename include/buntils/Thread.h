...
#pragma warning(disable : 4275)
  ...
  public:
    ...
    // Waits on the semaphore by waiting until it can decrement the value by 1 (which requires the semaphore is greater than
    // zero).
  ...
  public:
    ...
    BUN_FORCEINLINE size_t join(size_t mstimeout)
    {
      ...
      if(joinable())
      {
        ...
        {
          if(pthread_tryjoin_np(native_handle(), (void**)&ret) !=
             0) // If failed, thread is either still busy or something blew up, so return -1
            return (size_t)~0;
        }
        else
        {
          ...
          if(pthread_timedjoin_np(native_handle(), (void**)&ret, &ts) !=
             0) // size_t is defined as being big enough to hold a pointer
            return (size_t)~0;
        }
    ...
    Thread& operator=(Thread&& mov)
    {
      std::thread::operator=(std::move((std::thread&&)mov));
      return *this;
    }
  };
}
