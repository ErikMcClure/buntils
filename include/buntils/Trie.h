...
namespace bun {
  ...
  // Trie node
    ...
    struct BUN_COMPILER_DLLEXPORT TRIE_NODE
    {
  ...
  protected:
    ...
    BUN_FORCEINLINE void _checkSize(T r) { assert(r < (std::numeric_limits<T>::max() - 2)); if(r >= _capacity) { T s = _capacity; BASE::_setCapacity(_capacity << 1); _fill(s, _capacity); } }
