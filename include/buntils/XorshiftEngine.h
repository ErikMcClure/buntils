...
namespace bun {
  // Implementation of an xorshift64star generator. x serves as the generator state, which should initially be set to the
  // RNG seed.
  ...
  // Implementation of 2^1024-1 period xorshift generator. x is the 16*64 bit state, plus 1 extra integer for counting
  // indices.
  ...
  namespace internal {
    template<typename T> class xorshift_engine_base
    {
    public:
      BUN_FORCEINLINE static consteval T base_min() { return std::numeric_limits<T>::min(); }
      BUN_FORCEINLINE static consteval T base_max() { return std::numeric_limits<T>::max(); }
      BUN_FORCEINLINE static constexpr T base_transform(uint64_t x) { return (T)x; }
    };

    // DISABLED: There is no nice way of mapping randomly generated values to floating point values because floating point
    // has a log2 distribution and denormals. Instead, use gencanonical to map generated random integers to a [0.0,1.0) range.
    /*template<>
    ...
    y = (y&0xBFFFFFFF)+0x1F800000; // Mask out the top exponent bit to force exponent to 0-127 range, then add 63 to the
    exponent to get it in [63,190] range ([-64,63] when biased) return *(float*)(&y); // convert our integer into a float,
    assuming IEEE format
    }
    };
    ...
    {
    x = (x&0xBFFFFFFFFFFFFFFF)+0x1FF0000000000000; // Mask out the top exponent bit to force exponent to 0-1023 range, then
    add 511 to the exponent to get it in [511,1534] range ([-512,511] when biased) return *(double*)(&x); // convert our
    integer into a double, assuming IEEE format
    }
    };*/
  }

  template<typename T = uint64_t> class BUN_COMPILER_DLLEXPORT XorshiftEngine : protected internal::xorshift_engine_base<T>
  {
  public:
    XorshiftEngine() { seed(); }
    ...
    {
      return internal::xorshift_engine_base<T>::base_transform(xorshift1024star(_state));
    } // Truncate to return_value size.
