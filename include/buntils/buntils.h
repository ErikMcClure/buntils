...
namespace bun {
  ...
  template<typename, typename = void> constexpr bool is_copy_constructible_or_incomplete_v = true;

  template<typename T>
  constexpr bool is_copy_constructible_or_incomplete_v<T, std::void_t<decltype(sizeof(T))>> =
  ...
  template<class T> struct make_integral
  {
    using type = T;
  };
  ...
  // Get max size of an arbitrary number of bits, either signed or unsigned (assuming one's or two's complement
  // implementation)
  template<uint8_t BITS> struct BitLimit
  {
    static constexpr uint16_t BYTES = ((T_CHARGETMSB(BITS) >> 3) << (0 + ((BITS % 8) > 0))) + (BITS < 8);
    typedef typename std::conditional<
      sizeof(char) == BYTES, char, // rounds the type up if necessary.
      typename std::conditional<
        sizeof(int16_t) == BYTES, int16_t,
        typename std::conditional<sizeof(int32_t) == BYTES, int32_t,
                                  typename std::conditional<sizeof(int64_t) == BYTES, int64_t,
  #ifdef BUN_HASINT128
                                                            typename std::conditional<sizeof(__int128) == BYTES, __int128,
    ...
    static constexpr UNSIGNED UNSIGNED_MAX =
      (((UNSIGNED)2) << (BITS - 1)) -
      ((UNSIGNED)1); // these are all done carefully to ensure no overflow is ever utilized unless appropriate and it
                     // respects an arbitrary bit limit. We use 2<<(BITS-1) here to avoid shifting more bits than there are
                     // bits in the type.
    static constexpr SIGNED SIGNED_MIN_RAW =
      (SIGNED)(((UNSIGNED)1) << (BITS - 1)); // When we have normal bit lengths (8,16, etc) this will correctly result in a
                                             // negative value in two's complement.
    static constexpr UNSIGNED SIGNED_MIN_HELPER =
      (UNSIGNED)(((UNSIGNED)~0) << (BITS -
                                    1)); // However if we have unusual bit lengths (3,19, etc) the raw bit representation
                                         // will be technically correct in the context of that sized integer, but since we
                                         // have to round to a real integer size to represent the number, the literal
                                         // interpretation will be wrong. This yields the proper minimum value.
  ...
  template<typename T> struct TBitLimit : public BitLimit<sizeof(T) << 3>
  {};

  // Typesafe malloc implementation, for when you don't want to use New because you don't want constructors to be called.
  template<typename T> BUN_FORCEINLINE T* bun_Malloc(size_t sz) { return reinterpret_cast<T*>(malloc(sz * sizeof(T))); }
  ...
  template<int SZ> BUN_FORCEINLINE char* strcpyx0(char (&dst)[SZ], const char* src) { return strncpy(dst, src, SZ - 1); }
  #endif
  ...
  inline T IntDiv(T i, D div) noexcept
  {
    ...
    return (i / div) -
           ((i < 0) &
            ((i % div) != 0)); // If i is negative and has a nonzero remainder, subtract one to correct the truncation.
  }
  ...
  template<typename T> BUN_FORCEINLINE T* strtrim(T* str) noexcept { return strrtrim(strltrim(str)); }

  template<typename T> inline int ToArgV(T** argv, T* cmdline)
  {
    int count = 0;
    while(*cmdline)
    {
  ...
  // Processes an argv command line using the given function and divider. Use ToArgV if you only have a string (for windows
  // command lines)
  ...
  template<int I> BUN_FORCEINLINE void FlipEndian(std::byte* target) noexcept
  {
    if constexpr(I == 2)
  ...
  template<typename T> BUN_FORCEINLINE void FlipEndian(T* target) noexcept
  {
    FlipEndian<sizeof(T)>(reinterpret_cast<std::byte*>(target));
  }
  ...
  {
    return T_FBNEXT(x);
    // return x + 1 + (x>>1) + (x>>3) - (x>>7) + (x>>10) - (x>>13) - (x>>17) - (x>>21) + (x>>24); // 0.61803394 (but kind of
    // pointless)
  }
  ...
  template<typename T> BUN_FORCEINLINE constexpr char tsignzero(T n) noexcept { return (n > 0) - (n < 0); }
  ...
  // Smart compilers will use SSE2 instructions to eliminate the massive overhead of int -> float conversions. This uses
  // SSE2 instructions to round a float to an integer using whatever the rounding mode currently is (usually it will be
  // round-to-nearest)
  ...
  // Extremely fast rounding function that truncates properly, but only works in double precision mode; see
  ...
  // Single precision version of the above function. While precision problems are mostly masked in the above function by
  limiting it to
  // int32_t, in this function they are far more profound due to there only being 24 bits of mantissa to work with. Use with
  caution. BUN_FORCEINLINE int32_t fFastSingleRound(double val)
  {
  ...
  inline bool fWideCompare(float leftf, float rightf) noexcept
  {
    ...
    uint8_t dif = abs((0x7F800000 & left.i) - (0x7F800000 & right.i)) >>
                  23; // This grabs the 8 exponent bits and subtracts them.
    if(dif > 1)       // An exponent difference of 2 or greater means the numbers are different.
      return false;
    return !dif ? ((0x007FFF80 & left.i) == (0x007FFF80 & right.i)) :
                  !(abs((0x007FFF80 & left.i) - (0x007FFF80 & right.i)) -
                    0x007FFF80); // If there is no difference in exponent we tear off the last 7 bits and compare the value,
                                 // otherwise we tear off the last 7 bits, subtract, and then subtract the highest possible
                                 // significand to compensate for the extra exponent.
  }

  // Highly optimized traditional tolerance based approach to comparing floating point numbers, found here:
  ...
  {
    if(af == 0.0)
      return fSmall(bf, eps);
    if(bf == 0.0)
      return fSmall(af, eps);
  ...
  // This is a super fast length approximation for 2D coordinates; See http://www.azillionmonkeys.com/qed/sqroot.html for
  // details (Algorithm by Paul Hsieh)
  ...
  // The classic fast square root approximation, which is often mistakenly attributed to John Carmack. The algorithm is in
  // fact over 15 years old and no one knows where it came from.
  ...
  // Adaptation of the class fast square root approximation for double precision, based on
  ...
  BUN_FORCEINLINE T IntFastSqrt(T n) noexcept
  {
    return IntFastSqrt<T, sizeof(T) << 2>(n); // done to ensure loop gets unwound (the bit conversion here is <<2 because
                                              // the function wants HALF the bits in T, so <<3 >>1 -> <<2)
  }

  template<typename T> // assumes integer type if not one of the floating point types
  BUN_FORCEINLINE T FastSqrt(T n) noexcept
  {
    return IntFastSqrt(n);
  } // Picks correct method for calculating any square root quickly
  ...
  template<typename T> BUN_FORCEINLINE constexpr T DistSqr(T X, T Y, T x, T y) noexcept
  {
    T tx = X - x, ty = Y - y;
    return (tx * tx) + (ty * ty); // It doesn't matter if you use temporary values for floats, but it does if you use ints
                                  // (for unknown reasons)
  }

  // Distance calculation
  template<typename T> BUN_FORCEINLINE T Dist(T X, T Y, T x, T y) noexcept { return FastSqrt<T>(DistSqr<T>(X, Y, x, y)); }
  ...
  {
    if(!search || !length || !find || !fLength || length < fLength)
      return 0;
  ...
  // Counts the number of bits in v (up to 128-bit types) using the parallel method detailed here:
  // http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
  template<typename Tx>
    requires std::is_integral<typename make_integral<Tx>::type>::value
  ...
  BUN_FORCEINLINE std::pair<std::unique_ptr<T[]>, size_t> LoadFile(const char* file)
  {
    ...
    WFOPEN(f, file, "rb");
    if(!f)
      return std::pair<std::unique_ptr<T[]>, size_t>(nullptr, 0);
    fseek(f, 0, SEEK_END);
    size_t ln = ftell(f);
    fseek(f, 0, SEEK_SET);
  ...
  BUN_FORCEINLINE typename std::make_unsigned<T>::type bun_Abs(T x) noexcept
  {
    ...
    {
      T const mask = x >> ((sizeof(T) << 3) - 1); // Uses a bit twiddling hack to take absolute value without branching:
  ...
  namespace internal {
    ...
    template<class T> inline T _bunmultiplyextract(T xs, T ys, T shift) noexcept
    {
      ...
      if constexpr(std::is_signed<T>::value)
      {
        ...
        {
          high =
            ~high + !low; // only add one if the x addition would overflow, which can only happen if x is the maximum value
          low = ~low + 1;
        }
      }
      ...
      low  = (low >> shift);
      high = (high << ((sizeof(T) << 3) - shift)) &
             (-(shift > 0)); // shifting left by 64 bits is undefined, so we use a bit trick to set high to zero if shift is
                             // 0 without branching.
  ...
  template<> BUN_FORCEINLINE int64_t bun_MultiplyExtract<int64_t>(int64_t x, int64_t y, int64_t shift) noexcept
  {
    return internal::_bunmultiplyextract<int64_t>(x, y, shift);
  }
  template<> BUN_FORCEINLINE uint64_t bun_MultiplyExtract<uint64_t>(uint64_t x, uint64_t y, uint64_t shift) noexcept
  {
    return internal::_bunmultiplyextract<uint64_t>(x, y, shift);
  }
  #endif
  ...
  template<class T, class D = double> BUN_FORCEINLINE constexpr T lerp(T a, T b, D t) noexcept
  {
  ...
  inline constexpr uint64_t hash64_fnv1a(const char* const str, const uint64_t value = 0xcbf29ce484222325) noexcept
  {
    return (str[0] == '\0') ? value :
                              hash64_fnv1a(&str[1], uint32_t(uint64_t(value ^ uint64_t(str[0])) * 0x100000001b3ULL));
  }

  // template helper is_specialization_of, which can check if T is a specialization of any type that takes a variadic number
  // of arguments (variant, tuple, etc.)
  ...
  template<typename T> struct is_specialization_of_array : std::false_type
  {};
  template<typename T, int N> struct is_specialization_of_array<std::array<T, N>> : std::true_type
  {};
  ...
  template<typename T> BUN_FORCEINLINE void bun_Fill(T& p, unsigned char val = 0) { memset(&p, val, sizeof(T)); }
  template<sized_contiguous_range R> BUN_FORCEINLINE void bun_FillN(R&& p, unsigned char val = 0)
  {
    memset(std::ranges::data(p), val, sizeof(std::ranges::range_value_t<R>) * std::ranges::size(p));
  }

  template<class T, class SUBCLASS> void memsubset(T* p, int v)
  {
    memset(reinterpret_cast<char*>(p) + sizeof(SUBCLASS), v, sizeof(T) - sizeof(SUBCLASS));
  }
  template<class T, class SUBCLASS> void memsubcpy(T* dest, const T* src)
  {
    MEMCPY(((char*)dest) + sizeof(SUBCLASS), sizeof(T) - sizeof(SUBCLASS), ((char*)src) + sizeof(SUBCLASS),
           sizeof(T) - sizeof(SUBCLASS));
  }
  ...
  template<class _Ty> struct bun_DLLDelete
  ...
  template<class _Ty> struct bun_DLLDelete<_Ty[]>
...
//   struct int128
//   {
//     inline static int128& operator+=(const int128& right) {
//     _add(right.ints[0],right.ints[1],right.ints[2],right.ints[3]); return *this; }
//
//     int32_t ints[4];
//
//   private:
...
//   struct uint128
//   {
//     inline static uint128& operator+=(const uint128& right) {
//     _add(right.ints[0],right.ints[1],right.ints[2],right.ints[3]); return *this; }
//
//     uint32_t ints[4];
//
