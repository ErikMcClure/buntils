...
extern BUN_DLLEXPORT int itoa_r(int value, char* buffer, int size,
                                unsigned int radix); // For various stupid reasons we must reimplement multiple threadsafe
                                                     // versions of various functions because MinGW doesn't have them.
...
extern BUN_DLLEXPORT void SetWorkDirToCur(); // Sets the working directory to the actual goddamn location of the EXE instead
                                             // of the freaking start menu, or possibly the desktop. The possibilities are
                                             // endless! Fuck you, windows.
extern BUN_DLLEXPORT void ForceWin64Crash(); // I can't believe this function exists (forces 64-bit windows to not silently
                                             // ignore fatal errors)
...
template<int SZ> BUN_FORCEINLINE int _itoa_r(int value, char (&buffer)[SZ], unsigned int radix)
{
  return itoa_r(value, buffer, SZ, radix);
}
#endif
