...
#define BUN_VERSION_REVISION 3

// sometimes the std versions of these are a bit overboard, so this redefines the MS version, except it will no longer cause
// conflicts everywhere
#define bun_max(a, b)            (((a) > (b)) ? (a) : (b))
#define bun_min(a, b)            (((a) < (b)) ? (a) : (b))
#define bun_clamp(var, min, max) (((var) > (max)) ? (max) : (((var) < (min)) ? (min) : (var)))

// Use the following defines to force TODO items as warnings in the compile. VC++ currently does not support "messages"
// despite having them as a category in the error list. Usage: #pragma message(TODO "Clean up code here")
#define __TXT(x)  x
...
// These yield float/int ranges using rand(). However, rand() is a horrible RNG, so unless you have a good reason, use
// bun_rand() instead.
#ifndef RANDFLOATGEN
...
#define SAFESHIFT(v, s)       \
  (((s) > 0) ? ((v) << (s)) : \
               ((v) >> (-(s)))) // positive number shifts left, negative number shifts right, prevents undefined behavior.
#define T_GETBIT(type, bit) \
  ((type)(((type)1) << (((type)bit) % (sizeof(type) << 3)))) // Gets a bitmask using its 0-based index.
// #define T_GETBITRANGE(type, low, high)
// ((type)((((2<<((high)-(low)))-1)<<(low))|(((2<<((high)-(low)))-1)>>(-(low)))|(((2<<((high)-(low)))-1)<<((low)%(sizeof(type)<<3)))))
//  Gets a bitmask for a range of bits. This range can be negative to start from the left end instead of the right.
#define T_GETBITRANGE(type, low, high) \
  ((type)(SAFESHIFT(((2 << ((high) - (low))) - 1), low) | (((2 << ((high) - (low))) - 1) << ((low) % (sizeof(type) << 3)))))
#define MAKEVERSIONINT(major, minor, revision) \
  ((major) + ((minor) << 8) + ((revision) << 16)) // Packs version information into a 32-bit integer
// This calculates the most significant bit in an 8-bit integer (CHAR) at compile time. It's really just a basterdized
// version of i |= i >> 1; i |= i >> 2; i |= i >> 4; i -= i >> 1; so that it happens during compile time.
#define T_CHARGETMSB(i)                                                                             \
  ((((i | (i >> 1)) | ((i | (i >> 1)) >> 2)) | (((i | (i >> 1)) | ((i | (i >> 1)) >> 2)) >> 4)) -   \
   ((((i | (i >> 1)) | ((i | (i >> 1)) >> 2)) | (((i | (i >> 1)) | ((i | (i >> 1)) >> 2)) >> 4)) >> \
    1)) // I CAN'T BELIEVE THIS WORKS（ ﾟДﾟ）
// Round x up to next highest multiple of (t+1), which must be a multiple of 2. For example, to get the next multiple of 8:
// T_NEXTMULTIPLE(x,7)
...
#define GEN_FUNCTION_CHECK(FUNC)                                      \
  template<typename T, typename R, typename... Args> class has_##FUNC \
  {                                                                   \
...
#define DEFINE_MEMBER_CHECKER(Member)                         \
  ...
  #else
    #define TIME64(ptime) \
      time(ptime) // Linux does not have explicit 64-bit time functions, time_t will simply be 64-bit if the OS is 64-bit.
    #define GMTIMEFUNC(time, tm) \
      (gmtime_r(time, tm) == 0) // This makes it so it will return 1 on error and 0 otherwise, matching _gmtime64_s
    #define VSNWPRINTF(dst, size, format, list) \
      vswprintf(dst, size, format, list) // vswprintf is exactly what vsnwprintf should be, for some reason.
  #endif
  ...
  #define WCSTOK(str, delim, context) \
    wcstok(str, delim, context) // For some reason, in linux, wcstok *IS* the threadsafe version
