...
namespace bss_util {
  ...
  template<int I, int N>
  inline typename cSetting<I,N>::TYPE& Setting() { return cSetting<I,N>::v; }
...
#define DECL_SETGROUP(I,NAME) namespace bss_util { template<int N> class cSetting<I,N> { public: inline static const char* secname() { return NAME; } typedef void TYPE; }; }
  ...
  template<typename T>
  inline void cSetting_INILOAD(cINIstorage& ini, T& v, const char* name, const char* section)
  { 
    cINIentry* p;
  ...
  template<>
  inline void cSetting_INILOAD<std::vector<cStr>>(cINIstorage& ini, std::vector<cStr>& v, const char* name, const char* section)
  { 
    cINIentry* p;
    v.clear();
    if(name!=0) {
  ...
  template<>
  inline void cSetting_INILOAD<std::vector<int>>(cINIstorage& ini, std::vector<int>& v, const char* name, const char* section)
  { 
    cINIentry* p;
    v.clear();
    if(name!=0) {
  ...
  template<typename T>
  BSS_FORCEINLINE std::string cSetting_tostring(T& v)
  {
    std::stringstream s;
    s << v; 
    return s.str();
  ...
  template<typename T>
  inline void cSetting_INISAVE(cINIstorage& ini, T& v, const char* name, const char* section)
  { if(name!=0) ini.EditAddEntry(section,name,cSetting_tostring<T>(v).c_str()); }
  
  // Override for string types on INI loading
  template<>
  inline void cSetting_INISAVE<const char*>(cINIstorage& ini, const char*& v, const char* name, const char* section)
  { if(name!=0) { ini.EditAddEntry(section,name,v); } }
  // Force char types to write as numbers
  template<>
  inline void cSetting_INISAVE<unsigned char>(cINIstorage& ini, unsigned char& v, const char* name, const char* section)
  { char s[4]; ITOAx0((int)v,s,10); if(name!=0) { ini.EditAddEntry(section,name,s); } }
  template<>
  inline void cSetting_INISAVE<char>(cINIstorage& ini, char& v, const char* name, const char* section)
  { char s[4]; ITOAx0((int)v,s,10); if(name!=0) { ini.EditAddEntry(section,name,s); } }
  template<>
  ...
  {
    template<int U>
    struct __internal { static const int VAL=U+1; };
  ...
  public:
    ...
    inline static void LoadFromINI(cINIstorage& ini)
    {
      ...
      cSettingManage<I,std::conditional<std::is_void<typename cSetting<I,N+1>::TYPE>::value,__internal2,__internal<N>>::type::VAL>::LoadFromINI(ini);
    };
    ...
    inline static void SaveToINI(cINIstorage& ini)
    {
      ...
      cSettingManage<I,std::conditional<std::is_void<typename cSetting<I,N+1>::TYPE>::value,__internal2,__internal<N>>::type::VAL>::SaveToINI(ini);
    };
