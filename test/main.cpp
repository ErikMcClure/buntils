...
TESTDEF::RETPAIR test_VECTOR()
{
  ...
  // Test matrix transform functions
  {
    ...
    Matrix<float, 2, 2> af_ans ={ 0, -1, 0, 1 };
    Matrix<float, 2, 2> af = a;
    Matrix<float, 2, 2>::Rotation(PI_HALFf, d.v);
    Matrix<float, 2, 2>::Rotation_T(PI_HALFf, af);
    af.Transpose(af.v);
    TEST(af == d);
    TEST(MATRIX_COMPARE(af.v, af_ans.v, 10));

    Matrix<float, 3, 3> f1_ans ={ 0, -1, 0, 0, 1, 0, 0, 0, 1};
    Matrix<float, 3, 3> f1;
    Matrix<float, 3, 3> f2;
    Matrix<float, 3, 3> f3;
    ...
    Matrix<float, 3, 3>::Translation(4, 5, f1);
    f1.Inverse(f2.v);
    Matrix<float, 3, 3>::AffineRotation(3, f3);
    f1 = f2 * f3 * f1;
    Matrix<float, 3, 3>::Translation(1, 2, f2.v);
    f1 *= f2;
    Matrix<float, 3, 3>::AffineTransform(1, 2, 3, 4, 5, f3.v);
    TEST(MATRIX_COMPARE(f1.v, f3.v, 100));
    Matrix<float, 3, 3>::AffineTransform_T(1, 2, 3, 4, 5, f3.v);
    f1.Transpose(f2.v);
    TEST(MATRIX_COMPARE(f2.v, f3.v, 100));
