...
bool TEST_ALLOC_FUZZER_REMOVE(std::conditional_t<VERIFY, bss::Map<T*, size_t>, bss::DynArray<std::tuple<T*, size_t>>>& plist, A<T>& _alloc, int id)
{
  bool pass = true;
...
{
  bool pass = true;
  std::conditional_t<VERIFY, bss::Map<T*, size_t>, bss::DynArray<std::tuple<T*, size_t>>> plist;
...
void TEST_ALLOC_MT(TESTDEF::RETPAIR& __testret, Args... args)
{
  ...
  VARARRAY(bss::Thread, threads, NUM);
  for(int j = 0; j < 5; ++j)
  {
