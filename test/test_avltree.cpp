...
int avltestnum[8];
BUN_FORCEINLINE bool AVLACTION(AVLNode<int>* n)
{
  static int c    = 0;
  avltestnum[c++] = n->_key;
  return false;
...
TESTDEF::RETPAIR test_AVLTREE()
{
  ...
  AVLTree<int, int, std::compare_three_way, PolicyAllocator<AVLNode<std::pair<int, int>>, BlockPolicy>> avlblah(
    PolicyAllocator<AVLNode<std::pair<int, int>>, BlockPolicy>{ fixedavl });
  ...
  auto tree = new AVLTree<int, std::pair<int, int>*>(
    PolicyAllocator<AVLNode<std::pair<int, std::pair<int, int>*>>, BlockPolicy>{ fixedavlp });
  ...
  // DEBUG_CDT_SAFE<false>::_testret=&__testret; // Set things up so we can ensure AVLTree handles constructors/destructors
  // properly.
...

  {
    ...
    AVLTree<int, AVL_D, std::compare_three_way, PolicyAllocator<AVLNode<std::pair<int, AVL_D>>, BlockPolicy>> dtree(
      PolicyAllocator<AVLNode<std::pair<int, AVL_D>>, BlockPolicy>{ fixedavld });
    for(size_t i = 0; i < TESTNUM; ++i)
    {
      auto dp = dalloc.allocate(1);
      new(dp) DEBUG_CDT<false>(testnums[i]);
      dtree.Insert(testnums[i], AVL_D(dp, [&](DEBUG_CDT<false>* p) {
                     p->~DEBUG_CDT<false>();
                     dalloc.deallocate(p, 1);
                   }));
    }
  ...
  AVLTree<int, void, std::compare_three_way, PolicyAllocator<AVLNode<int>, BlockPolicy>> avlblah2{
    PolicyAllocator<AVLNode<int>, BlockPolicy>{ fixedavlkey }
  };
