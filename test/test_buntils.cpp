...
template<uint8_t B, int64_t SMIN, int64_t SMAX, uint64_t UMIN, uint64_t UMAX, typename T> inline void TEST_BitLimit()
{
  static_assert(std::is_same<T, typename BitLimit<B>::SIGNED>::value, "BitLimit failure" TXT(B));
  static_assert(std::is_same<typename std::make_unsigned<T>::type, typename BitLimit<B>::UNSIGNED>::value,
                "BitLimit failure" TXT(B));
...
template<class T> T naivebitcount(T v)
{
  T c;
  for(c = 0; v; v >>= 1)
    c += (v & 1);
...
template<class T> void testbitcount(TESTDEF::RETPAIR& __testret)
...
TESTDEF::RETPAIR test_buntils()
{
  ...
  static_assert(std::is_same<BitLimit<sizeof(uint32_t) << 3>::UNSIGNED, uint32_t>::value,
                "Test Failure Line #" TXT(__LINE__));
  ...
  static_assert(std::is_same<BitLimit<sizeof(double) << 3>::UNSIGNED, uint64_t>::value,
                "Test Failure Line #" TXT(__LINE__));
  static_assert(std::is_same<BitLimit<sizeof(float) << 3>::UNSIGNED, uint32_t>::value, "Test Failure Line #" TXT(__LINE__));
  // static_assert(std::is_same<BitLimit<sizeof(long double)<<3>::SIGNED, int64_t>::value, "Test Failure Line #"
  // TXT(__LINE__)); // long double is not a well defined type static_assert(std::is_same<BitLimit<sizeof(long
  // double)<<3>::UNSIGNED, uint64_t>::value, "Test Failure Line #" TXT(__LINE__));
  ...
  // Note that these tests CAN fail if trying to compile to an unsupported or buggy platform that doesn't have two's
  // complement.
  ...
  // These tests assume twos complement. This is ok because the previous tests would have caught errors relating to that
  // anyway.
  ...
  // Linux really hates wchar_t, so getting it to assign the right character is exceedingly difficult. We manually assign
  // 1585 instead.
  ...
  // This tests our average aggregation formula, which lets you average extremely large numbers while maintaining a fair
  // amount of precision.
  uint64_t total = 0;
  uint32_t nc;
  double avg  = 0;
...
#endif
  TEST(fabs(FastSqrt(2.0) - sqrt(2.0)) <=
       (DBL_EPSILON * 100)); // Take note of the 100 epsilon error here on the fastsqrt for doubles.
  uint32_t nmatch;
  ...
  // TEST(fFastDoubleRound(5.9f)==(int)5.9f); //This test fails, so don't use fFastDoubleRound for precision-critical
  // anything.
  ...
  TEST(Dist(2, 2, 5, 6) ==
       5); // Yes, you can actually do distance calculations using integers, since we use FastSqrt's integer extension.
  ...
  TEST(nmatch == 200000);
  for(nmatch = 2; nmatch < INT_MAX; nmatch <<= 1) // You have to do INT_MAX here even though its unsigned, because 10000...
                                                  // is actually less than 1111... and you overflow.
  {
  ...
  for(uint16_t i = 0; i <= 255; ++i)
    for(uint16_t j = 0; j <= 255; ++j)
      ...
      {
        TEST(_bunmultiplyextract<uint8_t>((uint8_t)i, (uint8_t)j, k) ==
             bun_MultiplyExtract<uint8_t>((uint8_t)i, (uint8_t)j, k));
      }
