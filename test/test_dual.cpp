...
{
  BEGINTEST;
  // d/dx( e^(x + x^2) + 2x/5 + cos(x*x) + sin(ln(x)+log(x)) ) = e^(x^2+x) (2 x+1)-2 x sin(x^2)+((1+log(10))
  // cos((1+1/(log(10))) log(x)))/(x log(10))+2/5
  ...
  for(int i = -10; i < 10; ++i)
  {
    ...
    if(!i)
      continue; // the functions below can't handle zeros.
    id = abs(i);
    ax = (adx ^ adx).cos();
    ...
    // TEST(ax(2)==(-((1+log(10.0))*((1+log(10.0))*sin((1+1/(log(10.0)))
    // log(id))+log(10.0)*cos((1+(1/(log(10.0))))*log(id))))/((id*id)*(log(10.0)*log(10.0)))));
  }
