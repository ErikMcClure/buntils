...
TESTDEF::RETPAIR test_HIGHPRECISIONTIMER()
{
  ...
  TEST(prof != 0);
  timer.ResetDelta(); // The linux profiler is *too accurate* in that it only measures precisely how much time the process
                      // is using. So we can't just put it to sleep.
