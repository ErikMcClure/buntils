...
template<class T> void _locklessqueue_consume(void* p)
{
...
template<class T> void _locklessqueue_produce(void* p)
{
  while(!startflag.load())
    ;
  T* q = (T*)p;
...
TESTDEF::RETPAIR test_LOCKLESSQUEUE()
{
  ...
  using LLQUEUE_SCSP = LocklessQueue<uint16_t, size_t>;
  {
    ...
    LLQUEUE_SCSP q{ PolicyAllocator<internal::LQ_QNode<uint16_t>, BlockPolicy>{
      policy } }; // single consumer single producer test
  ...
  for(size_t k = 0; k < 1; ++k)
  {
    ...
    for(size_t j = 2; j <= NUMTHREADS; j = fbnext(j))
    {
      ...
      LLQUEUE_MCMP q{ PolicyAllocator<internal::LQ_QNode<uint16_t>, LocklessBlockPolicy>{
        policy } }; // multi consumer multi producer test
      startflag.store(false);
