...
template<int M1, int M2, int M3, int M4> inline int PriCompare(const int (&li)[4], const int (&ri)[4])
{
  ...
  l += t;
  t = BUN_SSE_CAST_PS_SI128(
    BUN_SSE_MOVEHL_PS(BUN_SSE_CAST_SI128_PS(t), BUN_SSE_CAST_SI128_PS(l))); // Move upper 2 ints to bottom 2 ints
...
inline static uint32_t Interpolate(uint32_t l, uint32_t r, float c)
{
  ...
  // xl=_mm_shufflehi_epi16(xl,0xB1); // Now we have to Shuffle these values down because there is no way to convert an
  // uint32_t to a float. In any instruction set. Ever. BUN_SSE_M128 xfl = _mm_cvtepi32_ps(xl); // Convert to float
  ...
  // xl=_mm_shufflehi_epi16(xl,0xB1); // Shuffle the last two back up (this is actually the same Shuffle, since before we
  // just swapped locations, so we swap locations again and then we're back where we started). xl = _mm_and_si128(xl,xm); //
  // l&mask xr = xl; xr = _mm_shuffle_epi32(xr,0x1B); // Reverses the order of xr so we now have (d,c,b,a) xl =
  // _mm_or_si128(xl,xr); // Or xl and xr so we get (d|a,c|b,b|c,a|d) in xl xr =
  // _mm_castps_si128(_mm_movehl_ps(BUN_SSE_CAST_SI128_PS(xr),BUN_SSE_CAST_SI128_PS(xl))); // Move upper 2 ints to bottom 2
  // ints in xr so xr = (d,c,d|a,c|b) xl = _mm_or_si128(xl,xr); // Now or them again so we get (d|a,c|b,b|c | d|a,a|d | c|b)
  // which lets us take out the bottom integer as our result

  sseVeci xl(l); // duplicate l 4 times in the 128-bit register (l,l,l,l)
  sseVeci xm(0x000000FF, 0x0000FF00, 0x00FF0000,
             0xFF000000);                 // Channel masks (alpha,red,green,blue), these are loaded in reverse order.
  xl = sseVeci::ShuffleHi<0xB1>(xl & xm); // Now we have to Shuffle (l&m) down because there is no way to convert an
                                          // uint32_t xmm register to a float. In any instruction set. Ever.
  sseVec xc(c);                           // (c,c,c,c)
  ...
  xl =
    ((sseVec(xr) * xc) + (sseVec(xl) * (sseVec(1.0f) - xc))); // do the operation (r*c) + (l*(1.0-c)) across all 4 integers,
                                                              // converting to and from floating point in the process.
  xl = sseVeci::ShuffleHi<0xB1>(
    xl); // reverse our shuffling from before (this is actually the same Shuffle, since before we just swapped locations, so
         // we swap locations again, and then we're back where we started).
  ...
  xr = BUN_SSE_CAST_PS_SI128(
    BUN_SSE_MOVEHL_PS(BUN_SSE_CAST_SI128_PS(xr),
                      BUN_SSE_CAST_SI128_PS(xl))); // Move upper 2 ints to bottom 2 ints in xr so xr = (d,c,d|a,c|b)
  return (uint32_t)BUN_SSE_SI128_SI32(
    xl | xr); // Now OR them again so we get (d|a,c|b,b|c | d|a,a|d | c|b), then store the bottom 32-bit integer. What kind
              // of fucked up name is _mm_cvtsi128_si32 anyway?
}

static uint32_t flttoint(const float (&ch)[4])
{
  // return
  // (((uint32_t)(ch[0]*255.0f))<<24)|(((uint32_t)(ch[1]*255.0f))<<16)|(((uint32_t)(ch[2]*255.0f))<<8)|(((uint32_t)(ch[3]*255.0f)));
  sseVeci xch =
    ((BUN_SSE_M128i)BUN_SSE_SHUFFLEHI_EPI16(sseVeci(sseVec(ch) * sseVec(255.0f, 65280.0f, 255.0f, 65280.0f)), 0xB1));
  xch &= sseVeci(0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
  sseVeci xh =
    (BUN_SSE_M128i)BUN_SSE_SHUFFLE_EPI32((BUN_SSE_M128i)xch,
                                         0x1B); // assign the values of xl to xr, but reversed, so we have (d,c,b,a)
  xch |= xh;                                    // OR xl and xr so we get (d|a,c|b,b|c,a|d) in xl
  xh = BUN_SSE_CAST_PS_SI128(
    BUN_SSE_MOVEHL_PS(BUN_SSE_CAST_SI128_PS(xh),
                      BUN_SSE_CAST_SI128_PS(xch))); // Move upper 2 ints to bottom 2 ints in xr so xr = (d,c,d|a,c|b)
